            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <input name="date" type="datetime" date-type="date" class="form-control"  value="<?php echo Helpers_DateTime::getDateFormatRus(Arr::path($siteData->urlParams, 'date', ''));?>">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="shop_company_id">
                            Компания
                        </label>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="shop_bank_account_id">
                            № счета
                        </label>
                        <select data-type="select2" id="shop_bank_account_id" name="shop_bank_account_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_bank_account_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/bank/account/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="shop_commission_source_type_id">
                            Вид коммисии
                        </label>
                        <select data-type="select2" id="shop_commission_source_type_id" name="shop_commission_source_type_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_commission_source_type_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/commission/source/type/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
