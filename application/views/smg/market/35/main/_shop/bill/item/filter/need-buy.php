    ...
    <div class="row">
        <div class="col-md-12">
            <div class="row">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="shop_bill_item_status_id">Статус</label>
                        <select data-type="select2" id="shop_bill_item_status_id" name="shop_bill_item_status_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_bill_item_status_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="" data-id="" <?php if($tmp === ''){echo 'selected';} ?>>Все</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/bill/item/status/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="shop_bill_item_status_ids">Статусы</label>
                        <select data-type="select2" data-type="select2" id="shop_bill_item_status_ids" name="shop_bill_item_status_ids[]" multiple class="form-control select2" style="width: 100%;">
                            <?php $list = Request_RequestParams::getParamArray('shop_bill_item_status_ids', [], []); ?>
                            <?php
                            $view = $siteData->replaceDatas['view::_shop/bill/item/status/list/list'];
                            foreach ($list as $tmp) {
                                $tmp = 'data-id="' . $tmp . '"';
                                $view = str_replace($tmp, $tmp . ' selected', $view);
                            }
                            echo $view;
                            ?>
                        </select>
