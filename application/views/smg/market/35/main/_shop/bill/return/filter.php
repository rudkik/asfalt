                ...
                <div class="col-md-4">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <input id="date_from" class="form-control" name="date_from" value="<?php echo Helpers_DateTime::getDateFormatRus(Arr::path($siteData->urlParams, 'date_from', ''));?>" type="datetime" date-type="date">
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="name">Дата до</label>
                            <input id="date_to" class="form-control" name="date_to" value="<?php echo Helpers_DateTime::getDateFormatRus(Arr::path($siteData->urlParams, 'date_to', ''));?>" type="datetime" date-type="date">
                        </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="shop_bill_return_status_id">
                            Статус возврата заказа
                        </label>
                        <select id="shop_bill_return_status_id" name="shop_bill_return_status_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_bill_return_status_id'); ?>                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/bill/return/status/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
