...
<tr>
    <td>
        <input name="set-is-public" <?php echo $openPHP; ?> if ($data->values['is_public'] == 1) { echo ' value="1" checked'; }else{echo 'value="0"';} <?php echo $closePHP; ?> href="<?php echo $closePHP; ?> Func::getFullURL($siteData, '/<?php echo $pathViewStr; ?>/save', array('id' => 'id'), array(), $data->values); <?php echo $closePHP; ?>" type="checkbox" class="minimal" data-id="<?php echo $openPHP; ?> $data->id;  <?php echo $closePHP;?> ">
    </td>
    ...
    <?php  } if ($field['type'] == DB_FieldType::FIELD_TYPE_BOOLEAN) {?><td><?php echo $openPHP;?> if ($data->values['<?php echo $key; ?>'] == 1) { echo 'Да'; } <?php echo $closePHP; ?></td>
    <?php  } } } ?>
<td>
