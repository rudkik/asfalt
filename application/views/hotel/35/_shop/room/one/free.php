<tr data-price="<?php echo $data->values['price']; ?>">
    <td data-id="shop_room_id">
        ...
        <?php if($data->values['bedroom']){ ?>
            <img src="<?php echo $siteData->urlBasic; ?>/css/hotel/img/bedroom/<?php echo $data->values['bedroom']; ?>.png">
        <?php } ?>
        <?php if($data->values['two_bedroom']){ ?>
            <img src="<?php echo $siteData->urlBasic; ?>/css/hotel/img/two-bedroom/<?php echo $data->values['two_bedroom']; ?>.png">
        <?php } ?>
        <?php if($data->values['sofa']){ ?>
            <img src="<?php echo $siteData->urlBasic; ?>/css/hotel/img/sofa/<?php echo $data->values['sofa']; ?>.png">
        <?php } ?>
    </td>
    <?php foreach ($data->additionDatas['date'] as $key => $val){ ?>
        <td <?php if($val['paid_amount'] > 0){ ?>style="background-color: #0BDA51 !important;"<?php }if($val['price'] == $data->values['price_feast']){ ?>style="background-color: #fffae3"<?php }if($val['is_holiday']){?>style="background-color: #E3F7FF"<?php }?> data-action="select"
