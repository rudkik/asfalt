...
<form id="form-filter" class="box-body no-padding padding-bottom-10px">
	<div class="col-md-12">
		<div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label for="branch_move_id">Подразделение</label>
                    <select data-type="select2" id="branch_move_id" name="branch_move_id" class="form-control select2" required style="width: 100%;">
                        <option value="0" data-id="0">Выберите подразделение</option>
                        <?php
                        $tmp = 'data-id="' . Request_RequestParams::getParamInt('branch_move_id') . '"';
                        echo trim(str_replace($tmp, $tmp . ' selected', $siteData->replaceDatas['view::_shop/branch/move/list/list']));
                        ?>
                    </select>
                </div>
            </div>
