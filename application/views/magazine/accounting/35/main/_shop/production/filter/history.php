            ...
            <div class="row">
                <div class="col-md-5">
                    <div class="form-group">
                        <label for="shop_production_id">Продукт</label>
                        <select data-type="select2" id="shop_production_id" name="shop_production_id" class="form-control select2" style="width: 100%;">
                            <?php
                            $tmp = Request_RequestParams::getParamInt('shop_production_id');
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/production/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <script>
                    var option = $('#shop_production_id option:selected');
                    $('#production-name').text(option.text());
                </script>
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="created_at_from">Период от</label>
                        <input id="created_at_from" class="form-control" name="created_at_from" value="<?php echo Helpers_DateTime::getDateTimeFormatRus(Request_RequestParams::getParamDateTime('created_at_from'));?>" type="datetime" date-type="datetime">
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="created_at_to">Период до</label>
                        <input id="created_at_to" class="form-control" name="created_at_to" value="<?php echo Helpers_DateTime::getDateTimeFormatRus(Request_RequestParams::getParamDateTime('created_at_to'));?>" type="datetime" date-type="datetime">
                    </div>
                </div>
                <div class="col-md-1">
                    <div hidden>
