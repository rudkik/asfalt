<tr>
    ...
    <td>
        <a target="_blank" href="/bar/shopproduct/history?shop_product_id=<?php echo $data->values['id']; ?>">
    ...
    <td>
        <a target="_blank" href="/bar/shopproduction/edit?id=<?php echo $data->getElementValue('shop_production_id', 'id'); ?>">
            <?php echo $data->getElementValue('shop_production_id'); ?>
        </a>
        <br><?php echo $data->getElementValue('shop_production_id', 'barcode'); ?>
    </td>
    <td class="text-right" data-id="stock"><?php echo Func::getNumberStr($data->getElementValue('shop_product_stock_id', 'quantity_balance'), TRUE, 3, FALSE); ?></td>
    <td class="text-right"><?php echo Func::getNumberStr($data->values['price_cost'], TRUE, 2, FALSE); ?></td>
    <td>
        <ul class="list-inline tr-button <?php if ($data->values['is_delete'] == 1) { echo ' un-'; } ?>delete">
            ...
            <li><a data-action="calc-stock" href="<?php echo Func::getFullURL($siteData, '/shopproduct/calc_stocks', array(), array('shop_product_id' => $data->id), $data->values); ?>" class="link-blue"><i class="fa fa-calculator margin-r-5"></i> Пересчитать остатки</a></li>
        </ul>
    </td>
</tr>
