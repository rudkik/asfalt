...
<tr data-barcode="<?php echo $data->getElementValue('shop_production_id', 'barcode'); ?>" data-action="tr" <?php if($data->values['shop_realization_return_id'] > 0){ ?> class="tr-return"<?php }?>>
    ...
    <td>
        ...
        <input data-id="shop_production_id" name="shop_realization_return_items[<?php echo $data->id; ?>][shop_production_id]" value="<?php echo $data->values['shop_production_id']; ?>" hidden>
    </td>
    <td class="text-center">
        <div class="box-quantity">
            <input data-round="1" data-keywords="virtual" data-id="quantity" data-keywords="virtual" data-action="tr-multiply" data-total="#total" data-parent-count="3" name="shop_realization_return_items[<?php echo $data->id; ?>][quantity]" type="tel" class="form-control text-center" placeholder="Кол-во" required value="<?php echo Func::getNumberStr($data->values['quantity'], FALSE, 3); ?>" <?php if($isShow){ ?>readonly<?php }?>>
        </div>
        <div class="box-price">
    ...
    <td data-id="total" class="text-right" value="<?php echo $data->values['amount']; ?>">
        ...
        <input name="shop_realization_return_items[<?php echo $data->id; ?>][id]" value="<?php echo $data->id; ?>" style="display: none">
    </td>
    <?php if(!$isShow){ ?>
        <td class="text-center">
            <ul class="list-inline tr-button delete return">
