<h1>Заказ №<?php echo $data->values['id']; ?></h1>
    ...
    <div class="col-md-12">
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs ui-sortable-handle">
                ...
                <li class=""><a href="#info-legal" data-toggle="tab" aria-expanded="true">Юридические данные торговой точки</a></li>
                <li class=""><a href="#info-supplier" data-toggle="tab" aria-expanded="true">Данные поставщиками</a></li>
...
<div class="tab-content no-padding">
    <div class="tab-pane active" id="goods">
        <form method="post" action="/sadmin/shopbill/save">
            <div class="col-md-12">
            ...
            <div class="col-md-12">
                <div class="bill-save-cart">
                    <a target="_blank" href="/sadmin/shopbill/load_bill_in_excel?id=<?php echo $data->values['id']; ?>&shop_branch_id=<?php echo $data->values['shop_id']; ?>&file=bill.xlsx"  class="btn btn-warning">Выгрузка в Excel</a>
                </div>

                <div class="bill-save-bill" style="width: 495px;">
                    <div class="row">
                        <div class="col-md-12">
                            <a class="btn btn-info pull-right" data-id="7" data-action="set-bill-status" href="#">Заказ выполнен</a>
                            <?php if(($data->values['shop_bill_status_id'] != 7)){ ?>
                                <a class="btn btn-info pull-right" data-id="4" data-action="set-bill-status" href="#">Подтвердить заказ</a>
                            <?php } ?>
                            <a class="btn btn-danger pull-right" data-id="9" data-action="set-bill-status" href="#">Отменить заказ</a>
                        </div>
                    </div>

                    <div id="bill-comment" class="bill-comment" style="display: none">
                        <textarea class="form-control" name="data[admin_comment]" rows="4" placeholder="Примечание к заказу"></textarea>
    ...
    <div class="tab-pane body-partner-goods" id="info-supplier">
        <div class="col-md-12">
            <table class="table table-hover">
                <tbody>
                <tr>
                    ...
                    <td>
                        <?php
                        $phones = Arr::path($data->values, '$elements$.shop_id.options.site_phones', array());
                        if((is_array($phones)) && (count($phones))) {
                            $s = '';
                            foreach ($phones as $phone) {
                                $s = $s . trim(Arr::path($phone, 'phone', '') . ' ' . Arr::path($phone, 'info', '')) . ', ';
                ...
                <tr>
                    <td>E-mail</td>
                    <td><?php echo Arr::path($data->values, '$elements$.shop_id.options.site_email', ''); ?></td>
                </tr>
                <tr>
                    <td>Адрес</td>
                    <td><?php echo Arr::path($data->values, '$elements$.shop_id.options.site_address', ''); ?></td>
                </tr>
                <?php
                $map = Arr::path($data->values, '$elements$.shop_id.options.site_map', '');
                if(!empty($map)){
                    ?>
                    <tr>
                        <td>Карта</td>
                ...
                <tr>
                    <td>Прочее описание</td>
                    <td><?php echo Arr::path($data->values, '$elements$.shop_id.options.site_comment', ''); ?></td>
                </tr>
                </tbody>
            </table>
        </div>
