...
<script>
    function _initBookkeperTasks() {
        $('#bookkeepers [data-action="task-bookkeeper"]').on('click', function() {
            getTasks($(this).data('date'), $(this).data('bookkeeper'));
            return false;
        });
    }
    function getTasks(date, shopBookkeeperID) {
        jQuery.ajax({
            url: '/nur-admin/shoptaskcurrent/json_calendar_day',
            data: ({
                'date': (date),
            ...
            success: function (data) {
                ...
                jQuery.each(obj.tasks, function (index, value) {
                    ...
                    if(value.status == 1){
                        task = task +
                            '<td><a href="/nur-admin/shoptaskcurrent/finish?id='+value.shop_task_id+'&shop_branch_id='+value.shop_id+'"><span class="ks-icon fa fa-pencil-square-o"></span> Завершить работу</a></td>\n' +
                            '</tr>';
                    }else{
                        task = task +
                            '<td><a href="/nur-admin/shoptaskcurrent/add?id='+value.shop_task_id+'&shop_branch_id='+value.shop_id+'"><span class="ks-icon fa fa-pencil-square-o"></span> Взять в работу</a></td>\n' +
                            '</tr>';
                    }
                ...
                jQuery.each(obj.bookkeepers, function (index, value) {
                    var bookkeeper = '<tr>\n' +
                        '<td><a data-action="task-bookkeeper" data-date="'+date+'" data-bookkeeper="'+value.shop_bookkeeper_id+'" href="#">'+value.shop_bookkeeper_name+'</a></td>\n' +
                        '<td>'+value.count+'</td>\n' +
                        '</tr>';
    ...
    $(function () {
        $('#calendar').fullCalendar({
            ...
            eventLimit: true,
            events: {
                url: '/nur-admin/shoptaskcurrent/json_calendar',
                error: function() {
                }
            },
            editable: false,
