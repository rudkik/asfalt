<div class="ks-nav-body">
    ...
    <table id="task-current-data-table" data-action="nur-table" class="table table-striped table-bordered" width="100%"
           ...
           data-url="<?php echo $siteData->urlBasic; ?>/nur-bookkeeping/shoptaskcurrent/json_new?period=<?php echo Request_RequestParams::getParamStr('period'); ?>&is_total=1&_fields[]=date_from&_fields[]=date_to&_fields[]=name&_fields[]=shop_bookkeeper_name&_fields[]=shop_name&_fields[]=id"
...
<script>
    function getEditURL(value, row) {
        return '<a href="/nur-bookkeeping/shoptaskcurrent/add?id='+ row['shop_task_id']+'&shop_branch_id='+row['shop_id']+'"><span class="ks-icon fa fa-pencil-square-o"></span> Взять в работу</a>';
    }
</script>
