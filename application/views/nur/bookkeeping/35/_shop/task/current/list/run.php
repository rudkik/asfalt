<div class="ks-nav-body">
    <div id="task-current-toolbar" class="ks-datatable-toolbar">
        <div class="ks-items-block">
            ...
            <button data-action="table-delete" data-table="#task-current-data-table" data-url="<?php echo $siteData->urlBasic; ?>/nur-bookkeeping/shoptaskcurrent/del" class="btn btn-danger ks-solid"><span class="ks-icon fa fa-remove "></span> <span class="ks-text">Удалить</span></button>
        </div>
    </div>
    ...
    <table id="task-current-data-table" data-action="nur-table" class="table table-striped table-bordered" width="100%"
           ...
           data-url="<?php echo $siteData->urlBasic; ?>/nur-bookkeeping/shoptaskcurrent/json_run?is_total=1&_fields[]=shop_id&_fields[]=date_start&_fields[]=name&_fields[]=shop_bookkeeper_name&_fields[]=shop_name&_fields[]=date_finish&_fields[]=id"
...
<script>
    function getEditURL(value, row) {
        return '<a href="/nur-bookkeeping/shoptaskcurrent/finish?id='+ row['id']+'&shop_branch_id='+row['shop_id']+'"><span class="ks-icon fa fa-pencil-square-o"></span> Завершить работу</a>';
    }
</script>
