    ...
    <div class="row">
        <div class="col-md-12">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="formula_type_id">Вид рецепта</label>
                        <select id="formula_type_id" name="formula_type_id" class="form-control select2" style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('formula_type_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::formula-type/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
