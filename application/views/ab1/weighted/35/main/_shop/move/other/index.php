	...
	<div class="nav-tabs-custom nav-tabs-navy">
        ...
        <div class="tab-content">
            <div class="tab-pane active">
                ...
                <?php
                $view = View::factory('ab1/weighted/35/main/_shop/move/other/filter');
                $view->siteData = $siteData;
                $view->data = $data;
                echo Helpers_View::viewToStr($view);
                ?>
            ...
            <div class="nav-tabs-custom" style="margin-bottom: 0px;">
                <ul id="tab-status" class="nav nav-tabs pull-right ui-sortable-handle">
                    <li class="<?php if(Arr::path($siteData->urlParams, 'is_delete', '') == 1){echo 'active';}?>"><a href="<?php echo Func::getFullURL($siteData, '/shopmoveother/index', array(), array('is_delete' => 1, 'is_public_ignore' => 1));?>" data-id="is_delete_public_ignore">Удаленные</a></li>
                    <li class="<?php if((Arr::path($siteData->urlParams, 'is_delete', '') != 1) && (Arr::path($siteData->urlParams, 'is_not_public', '') != 1) && (Arr::path($siteData->urlParams, 'is_public', '') != 1)){echo 'active';}?>"><a href="<?php echo Func::getFullURL($siteData, '/shopmoveother/index', array(), array('is_public_ignore' => 1));?>"  data-id="is_public_ignore">Все <i class="fa fa-fw fa-info text-blue"></i></a></li>
                    <li class="pull-left header">
                        <a data-action="new-auto" href="<?php echo Func::getFullURL($siteData, '/shopmoveother/new', array());?>" class="btn bg-purple btn-flat">
            ...
            <div class="box-body table-responsive" style="padding-top: 0px;">
                <?php echo trim($data['view::_shop/move/other/list/index']); ?>
            </div>
            </div>
        </div>
	</div>
    ...
    function PrintTTN(id, pages) {
        jQuery.ajax({
            ...
            data: ({
                'action': ('print'),
                'type': ('ttn_material'),
                'table_id': ('<?php echo Model_Ab1_Shop_Move_Other::TABLE_ID; ?>'),
                'pages': (pages),
                'id': (id)
            }),
            type: "GET",
    ...
    $('[data-action="new-auto"]').click(function() {
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                window.location.href = ('<?php echo Func::getFullURL($siteData, '/shopmoveother/new', array());?>'+'?number='+obj.number+'&weight='+obj.weight);
            },
            error: function (data) {
                console.log(data.responseText);
