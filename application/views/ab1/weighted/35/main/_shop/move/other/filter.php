			...
			<div class="row">
				...
				<div class="col-md-2">
					<div class="form-group">
						<label for="shop_material_other_id">Материал</label>
                        <select id="shop_material_other_id" name="shop_material_other_id" class="form-control select2" style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_material_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>></option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/material/other/list/list']));
                            ?>
                        </select>
					</div>
				</div>
                ...
                <?php }else{ ?>
                <div class="col-md-2" style="height: 76.85px">
                </div>
                <?php } ?>
