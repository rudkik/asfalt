...
if($siteData->operation->getIsAdmin()){ ?>
<div class="row record-input record-list">
    ...
    <div class="col-md-3">
        <input name="created_at" type="datetime" date-type="datetime" class="form-control" value="<?php echo Helpers_DateTime::getDateTimeFormatRus($data->values['created_at']);?>">
    </div>
    <div class="col-md-3 record-title">
        <label>
            Дата выезда
    ...
    <div class="col-md-3">
        <input name="weighted_exit_at" type="datetime" date-type="datetime" class="form-control" value="<?php echo Helpers_DateTime::getDateTimeFormatRus($data->values['weighted_exit_at']);?>">
    </div>
</div>
<?php }?>
<div class="row record-input record-list">
...
<div class="row record-input record-list">
    ...
    <div class="col-md-3">
        <select id="shop_material_other_id" name="shop_material_other_id" class="form-control select2" required style="width: 100%;">
            <option value="0" data-id="0">Без значения</option>
...
<?php if($siteData->action != 'clone') { ?>
    <div class="row record-input record-list">
        ...
        <div class="col-md-9">
            <input data-type="money" data-fractional-length="3" id="quantity" name="quantity" type="text"  class="text-number form-control" placeholder="Вес" required value="<?php echo Func::getNumberStr($data->values['quantity'], true, 3); ?>" <?php if(!$siteData->operation->getIsAdmin()) { ?> disabled<?php } ?>>
        </div>
    </div>
<?php }else{ ?>
    <div class="row record-input record-list">
...
<script>
    function submitCar(id) {
        ...
        var element = $('#'+id+' [name="shop_material_id"]');
        var element1 = $('#'+id+' [name="shop_material_other_id"]');
        if ((!$.isNumeric(element.val()) || parseInt(element.val()) < 1)
        && (!$.isNumeric(element1.val()) || parseInt(element1.val()) < 1)){
            element.parent().addClass('has-error');
