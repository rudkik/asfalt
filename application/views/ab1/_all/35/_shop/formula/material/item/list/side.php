...
<div class="row record-input record-list">
    ...
    <div class="col-md-9">
        <table class="table table-hover table-db table-tr-line" >
            ...
            <tbody id="raws-side">
            <?php
            foreach ($data['view::_shop/formula/material/item/one/side']->childs as $value) {
                echo $value->str;
            }
            ?>
            </tbody>
        ...
        <?php if(!$isShow){ ?>
            <div class="modal-footer text-center">
                <button type="button" class="btn btn-danger" onclick="addElement('new-raw-side', 'raws-side', true);">Добавить строчку</button>
            </div>
        <?php } ?>
    </div>
</div>
<?php if(!$isShow){ ?>
<div id="new-raw-side" data-index="0">
    <!--
    <tr>
        <td>
            <select data-id="raw" name="shop_formula_sides[_#index#][shop_material_id]" class="form-control select2" required style="width: 100%;">
