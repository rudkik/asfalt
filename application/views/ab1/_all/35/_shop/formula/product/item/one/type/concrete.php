...
<?php if(Arr::path($data->values['options'], 'is_liter', '0') == 0){ ?>
    <tr>
        ...
        <td>
            <input data-type="money" data-fractional-length="3" data-action="calc-asphalt" data-id="losses" name="shop_formula_items[<?php echo $data->id; ?>][losses]" type="text" class="form-control text-right" placeholder="Потери (%)" required value="<?php echo Func::getNumberStr($data->values['losses'], false, 10, true); ?>" <?php if($isShow){ ?>readonly<?php } ?>>
        </td>
        <td>
            <input data-type="money" data-fractional-length="3" data-id="norm_industrial" name="shop_formula_items[<?php echo $data->id; ?>][options][norm_industrial]" type="text" class="form-control text-right" placeholder="Производственные нормы (кг)" required value="<?php echo Arr::path($data->values['options'], 'norm_industrial', ''); ?>" readonly>
        </td>
        <td class="text-center"> - </td>
        <?php if(!$isShow){ ?>
            <td>
...
<?php }else{ ?>
    <tr style="background-color: rgba(97, 106, 168, 0.3);">
        ...
        <td class="text-center"> - </td>
        <td>
            <input data-type="money" data-fractional-length="3" data-action="calc-asphalt" data-id="norm_weight" name="shop_formula_items[<?php echo $data->id; ?>][norm_weight]" type="text" class="form-control text-right" placeholder="Расход (л)" required value="<?php echo Func::getNumberStr($data->values['norm_weight'], false, 10, true); ?>" <?php if($isShow){ ?>readonly<?php } ?>>
        </td>
        <td>
            <input data-type="money" data-action="calc-asphalt" data-id="losses" name="shop_formula_items[<?php echo $data->id; ?>][losses]" type="text" class="form-control text-right" placeholder="Потери (%)" required value="<?php echo Func::getNumberStr($data->values['losses'], false, 10, true); ?>" readonly>
        </td>
        <td class="text-center"> - </td>
        <td>
            <input data-type="money" data-id="norm_industrial" name="shop_formula_items[<?php echo $data->id; ?>][options][norm_industrial]" type="text" class="form-control text-right" placeholder="Производственные нормы (л)" required value="<?php echo Arr::path($data->values['options'], 'norm_industrial', ''); ?>" readonly>
        </td>
        <?php if(!$isShow){ ?>
        <td>
