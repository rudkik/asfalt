...
<div class="row record-input record-list">
    ...
    <div class="col-md-3">
        <input name="from_at" type="datetime" date-type="date" class="form-control" placeholder="Начало" required value="<?php echo Helpers_DateTime::getDateTimeRusWithoutSeconds(Arr::path($data->values, 'from_at', ''));?>" <?php if($isShow){ ?>readonly<?php } ?>>
    </div>
    <div class="col-md-3 record-title">
        <label>
            <sup><i class="fa fa-fw fa-asterisk text-red"></i></sup>
    ...
    <div class="col-md-3">
        <input name="to_at" type="datetime" date-type="date" class="form-control" placeholder="Окончание" required value="<?php echo Helpers_DateTime::getDateTimeRusWithoutSeconds(Arr::path($data->values, 'to_at', ''));?>" <?php if($isShow){ ?>readonly<?php } ?>>
    </div>
</div>
<div class="row record-input record-list">
    <div class="col-md-3 record-title">
    ...
    <div class="row record-input record-list">
        ...
        <div class="col-md-9">
            <textarea rows="3" name="text" type="text" class="form-control" placeholder="Комментарий" <?php if($isShow){ ?>readonly<?php } ?>><?php echo Arr::path($data->values, 'text', '');?></textarea>
        </div>
    </div>
<?php if(!$isShow){ ?>
<div class="row">
