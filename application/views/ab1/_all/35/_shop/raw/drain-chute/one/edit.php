<?php $isShow = !$siteData->operation->getIsAdmin() && Request_RequestParams::getParamBoolean('is_show'); ?>
    ...
    <div class="row record-input record-list">
        ...
        <div class="col-md-3">
            <input data-type="money" data-fractional-length="3" name="size_ton" type="text" class="form-control" placeholder="Размер в тоннах" required value="<?php echo htmlspecialchars($data->values['size_ton'], ENT_QUOTES);?>" <?php if($isShow){ ?>readonly<?php } ?>>
        </div>
    </div>
<?php if(!$isShow){ ?>
    <div class="row">
