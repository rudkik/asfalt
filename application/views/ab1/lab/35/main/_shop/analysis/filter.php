            ...
            <div class="row">
                <div class="col-md-1-5">
                    <div class="form-group">
                        ...
                        <input name="date_from" type="datetime"  date-type="datetime"  class="form-control" value="<?php echo Arr::path($siteData->urlParams, 'date_from', '');?>">
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        <label for="date_to">Период до</label>
                        <input name="date_to" type="datetime"  date-type="datetime"  class="form-control" value="<?php echo Arr::path($siteData->urlParams, 'date_to', '');?>">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="shop_analysis_type_id">Вид испытания</label>
                        <select id="shop_analysis_type_id" name="shop_analysis_type_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_analysis_type_id'); ?>
                            ...
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/analysis/type/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="shop_analysis_place_id">Место испытания</label>
                        <select id="shop_analysis_place_id" name="shop_analysis_place_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_analysis_place_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/analysis/place/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select id="shop_raw_id" name="shop_raw_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_raw_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="shop_product_id">Продукция</label>
