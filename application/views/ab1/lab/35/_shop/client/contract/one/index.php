<?php
$isMaterialRaw = Request_RequestParams::getParamBoolean('is_material_raw');
...
<tr <?php if($data->values['is_basic'] == 0){ ?>class="additoin-agreement"<?php } ?>>
    ...
    <td>
        <ul class="list-inline tr-button <?php if ($data->values['is_delete'] == 1) { echo ' un-'; } ?>delete">
            ...
            $button = 'Изменить';
            $isDelete = true;
            if(Request_RequestParams::getParamBoolean('is_material_raw') && !$siteData->operation->getIsAdmin()){
                $fixed['is_show'] = 1;
                $button = 'Просмотр';
                $isDelete = false;
            }
