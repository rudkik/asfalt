                ...
                <table class="table table-hover table-db table-tr-line" >
                    ...
                    <tr>
                        <?php foreach ($child['fields'] as $field) {
                            ...
                            ?>
                            <td>
                            <?php if ($field['type'] == Model_Ab1_Shop_Analysis_Type::OPTIONS_TYPE_TABLE) {?>
                            ...
                            <?php }else{ if ($field['quantity_value'] == ''){?>
                                <input name="options[<?php echo $title; ?>]" <?php if ($field['type'] == Model_Ab1_Shop_Analysis_Type::OPTIONS_TYPE_DATE) { ?> type="datetime" date-type="date"  <?php } ?>
                                    ...
                                    <?php if (!empty($field['formula'])){ ?> type="text" readonly <?php }?> class="form-control" value="<?php echo $data->values['options'][$title] ?>"  data-variable="<?php echo $field['variable']; ?>" data-formula="<?php echo $field['formula']; ?>">
                            <?php }else{
                                for ($i=0; $i < $field['quantity_value']; $i++){ ?>
                                    <div class="col-md-<?php echo ceil(12 / ($field['quantity_value'] + 1)); ?>" style="padding:0 1px">
                                        <input type="phone"  name="options[<?php echo $title; ?>][value][]" data-all="<?php echo $title; ?>" class="form-control" value="<?php echo $data->values['options'][$title]['value'][$i]; ?>">
                                    </div>
                                <?php } ?>
                                <div class="col-md-<?php echo ceil(12 / ($field['quantity_value'] + 1)); ?>" style="padding:0 1px">
                                    <input name="options[<?php echo $title; ?>][total]" type="phone" data-total="<?php echo $title; ?>" value="<?php echo $data->values['options'][$title]['total']; ?>" class="form-control" style="background-color: #eee">
                                </div>
                            <?php }}?>
                            </td>
                        <?php } ?>
            ...
            <div class="col-md-3">
                ...
                <?php }else { if ($child['field']['quantity_value'] == ''){?>
                    <input name="options[<?php echo $title; ?>]" <?php if ($type == Model_Ab1_Shop_Analysis_Type::OPTIONS_TYPE_DATE) { ?> type="datetime" date-type="date" <?php } ?>
                        ...
                        <?php if(($child['field']['formula']) != ''){ ?> type="text" readonly data-formula="<?php echo $child['field']['formula']; ?>" <?php } ?> class="form-control"  data-variable="<?php echo $child['field']['variable']; ?>" value="<?php echo $data->values['options'][$title] ?>">
                <?php }else{
