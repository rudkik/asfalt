    ...
    <div class="row">
        <div class="col-md-12">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="shop_transport_mark_id">Транспорт</label>
                        <select id="shop_transport_mark_id" name="shop_transport_mark_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_transport_mark_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/transport/mark/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="shop_move_client_id">Подразделение</label>
                        <select id="shop_move_client_id" name="shop_move_client_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('shop_move_client_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/move/client/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        ...
                        <input name="date_from_equally" type="datetime"  date-type="date"  class="form-control" placeholder="Дата" value="<?php echo Arr::path($siteData->urlParams, 'date_from_equally', '');?>">
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        <label for="date_to">Период до</label>
                        <input name="date_to" type="datetime"  date-type="date"  class="form-control" placeholder="Дата" value="<?php echo Arr::path($siteData->urlParams, 'date_to', '');?>">
                    </div>
                </div>
