            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="season_id">Сезоны</label>
                        <select id="season_id" name="season_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('season_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::season/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        <label for="from_at">Период от</label>
                        <input name="from_at" type="datetime"  date-type="date"  class="form-control" placeholder="Дата" value="<?php echo Arr::path($siteData->urlParams, 'from_at', '');?>">
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        <label for="to_at">Период до</label>
                        <input name="to_at" type="datetime"  date-type="date"  class="form-control" placeholder="Дата" value="<?php echo Arr::path($siteData->urlParams, 'to_at', '');?>">
                    </div>
                </div>
                <div class="col-md-5">
                    <div hidden>
