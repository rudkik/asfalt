<?php $isMaterialRaw = Request_RequestParams::getParamBoolean('is_material_raw'); ?>
<table class="table table-hover table-db table-tr-line" data-action="fixed">
    <tr>
        ...
        <?php if(!$isMaterialRaw){ ?>
        <th class="width-110">
            <a href="<?php echo Func::getFullURL($siteData, '/shopclientcontract/index'). Func::getAddURLSortBy($siteData->urlParams, 'is_redaction_client'); ?>" class="link-black">Редакция договора</a>
        </th>
        <?php } ?>
        <?php if(!$isMaterialRaw){ ?>
    ...
    <form id="form-filter_" action="<?php echo Func::getFullURL($siteData, '/shopclientcontract/index', array());?>">
        ...
        <?php if(!$isMaterialRaw){ ?>
        <td style="min-width: 113px;">
            <select name="is_redaction_client" class="form-control select2" style="width: 100%;">
                <option value="">Выберите</option>
                <option value="1" <?php if(Request_RequestParams::getParamBoolean('is_redaction_client') === true){echo 'selected';} ?>>Контрагент</option>
                <option value="0" <?php if(Request_RequestParams::getParamBoolean('is_redaction_client') === false){echo 'selected';} ?>>АБ1</option>
            </select>
        </td>
        <?php } ?>
        <?php if(!$isMaterialRaw){ ?>
