<form id="form-filter" class="box-body no-padding padding-bottom-10px">
	...
	<div class="col-md-12">
		<div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label for="main_shop_product_rubric_id">Рубрика</label>
                    <select id="main_shop_product_rubric_id" name="main_shop_product_rubric_id" class="form-control select2" style="width: 100%;">
                        <?php $tmp = Request_RequestParams::getParamInt('main_shop_product_rubric_id'); ?>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label for="shop_product_pricelist_rubric_id">Рубрика прайс-листа</label>
                    <select id="shop_product_pricelist_rubric_id" name="shop_product_pricelist_rubric_id" class="form-control select2" style="width: 100%;">
                        <?php $tmp = Request_RequestParams::getParamInt('shop_product_pricelist_rubric_id'); ?>
                        <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>></option>
                        <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                        <?php
                        $tmp = 'data-id="'.$tmp.'"';
                        echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::_shop/product/pricelist/rubric/list/list']));
                        ?>
                    </select>
                </div>
            </div>
