			...
			<div class="row">
				<div class="col-md-3">
					<div class="form-group">
                        ...
                        <input id="input-name" class="form-control" name="name" placeholder="Клиент" value="<?php echo htmlspecialchars(Arr::path($siteData->urlParams, 'name', ''), ENT_QUOTES);?>" type="text">
					</div>
				</div>
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="bin">БИН/ИИН</label>
                        <input id="bin" class="form-control" name="bin" placeholder="БИН/ИИН" value="<?php echo Arr::path($siteData->urlParams, 'bin', '');?>" type="text">
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        ...
                        <select id="organization_type_id" name="organization_type_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('organization_type_id'); ?>
                            ...
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::organizationtype/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="kato_id">КАТО</label>
                        <select id="kato_id" name="kato_id" class="form-control select2" required style="width: 100%;">
                            <?php $tmp = Request_RequestParams::getParamInt('kato_id'); ?>
                            <option value="-1" data-id="-1" <?php if($tmp === NULL || $tmp < 0){echo 'selected';} ?>>Выберите значение</option>
                            <option value="0" data-id="0" <?php if($tmp == 0 && $tmp !== NULL){echo 'selected';} ?>>Без значения</option>
                            <?php
                            $tmp = 'data-id="'.$tmp.'"';
                            echo trim(str_replace($tmp, $tmp.' selected', $siteData->replaceDatas['view::kato/list/list']));
                            ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <input id="phones" class="form-control" name="phones" placeholder="Телефон" value="<?php echo Arr::path($siteData->urlParams, 'phones', '');?>" type="text">
                    </div>
                </div>
                <div class="col-md-1-5">
                    <div class="form-group">
                        <label class="span-checkbox" style="margin-top: 32px">
                            <input data-not-check="1" name="is_buyer" data-id="1" <?php if(Request_RequestParams::getParamBoolean('is_buyer')){ ?>value="1" checked<?php }else{ ?> value="0"<?php } ?> type="checkbox" class="minimal">
                            Покупатель
                        </label>
                    </div>
                </div>
