...
<?php }else{ ?>
    <tr>
        ...
        <td class="text-right">
            <?php
            $diff = round($data->values['price'] - $data->getElementValue('shop_product_time_price_id', 'price', 0), 2);
            ...
            if($data->values['shop_client_balance_day_id'] > 0){ ?>
                <a href="<?php echo Func::getFullURL($siteData, '/shopinvoice/set_current_price', array('shop_product_id' => 'shop_product_id'), array($field => Arr::path($data->values, $field, 0)), $data->values, true);?>" class="text-right text-blue" style="font-size: 12px;">Текущие цены</a>
                <br>
            <?php }else{ ?>
                <a href="<?php echo Func::getFullURL($siteData, '/shopinvoice/break_price_client_balance', array('shop_product_id' => 'shop_product_id'), array($field => Arr::path($data->values, $field, 0)), $data->values, true);?>" class="text-right text-blue" style="font-size: 12px;">Цены по фикс. балансам</a>
                <br>
            <?php }?>
            <?php if($siteData->operation->getIsAdmin()){ ?>
                <a href="<?php echo Func::getFullURL($siteData, '/shopinvoice/del_client_balance_days', array('shop_product_id' => 'shop_product_id'), array($field => Arr::path($data->values, $field, 0)), $data->values, true);?>" class="text-right text-red" style="font-size: 12px;">Не учитывать сумму</a>
            <?php }?>
        </td>
        <td class="text-right"><?php echo Func::getNumberStr($data->values['quantity'], TRUE, 3, FALSE); ?></td>
        <td class="text-right"><?php echo Func::getNumberStr($data->values['amount'], TRUE, 2, FALSE); ?></td>
        <td>
            <ul class="list-inline tr-button">
                <?php if($isAll){?>
                    <li><a href="<?php echo Func::getFullURL($siteData, '/'.$fieldURL.'/edit', array(), array('id' => Arr::path($data->values, $field, 0)), $data->values); ?>" class="link-blue"><i class="fa fa-edit margin-r-5"></i> Талон</a></li>
                <?php }?>
                <li>
