...
<head>
    ...
    <?php if($siteData->shopID > 0){ ?>
        <title><?php echo $siteData->siteTitle. ' - ' . $siteData->shop->getName(); ?></title>
    <?php }else{ ?>
...
<script async>
    $(function(){
        ...
        var testCashBox=function(){
            jQuery.ajax({
                url: '/zhbibc/shopfinish/aura3_status',
                data: ({}),
                type: "GET",
                success: function (data) {
                    var obj = jQuery.parseJSON($.trim(data));

                    $('#cash-box-panel').removeClass('bg-red');
                    $('#cash-box-panel').removeClass('bg-yellow');
                    $('#cash-box-panel').removeClass('bg-blue');
                    $('#cash-box-panel').removeClass('bg-orange');
                    if (obj.is_send_local != 1){
                        $('#cash-box-panel').addClass('bg-red');
                        $('#cash-box-status').text('отсутствует связь с регистратором');
                    }else {
                        if (obj.Coveropen == 1) {
                            $('#cash-box-panel').addClass('bg-yellow');
                            $('#cash-box-status').text('открыта крышка');
                        } else {
                            if (obj.Paperpresent == 1) {
                                $('#cash-box-panel').addClass('bg-yellow');
                                $('#cash-box-status').text('отсутствует бумага');
                            } else {
                                if (obj.Shiftopen == 0) {
                                    $('#cash-box-panel').addClass('bg-yellow');
                                    $('#cash-box-status').text('смена не открыта');
                                } else {
                                    if (obj.Mode == 1) {
                                        $('#cash-box-panel').addClass('bg-orange');
                                        $('#cash-box-status').text('в режиме OFFLINE');
                                    } else {
                                        if (obj.Mode == 2) {
                                            $('#cash-box-panel').addClass('bg-red');
                                            $('#cash-box-status').text('в режиме BLOCKED');
                                        } else {
                                            if (obj.Mode == 3) {
                                                $('#cash-box-panel').addClass('bg-red');
                                                $('#cash-box-status').text('в режиме SERVER_BLOCKED');
                                            } else {
                                                if (obj.Demomode == 1) {
                                                    $('#cash-box-panel').addClass('bg-orange');
                                                    $('#cash-box-status').text('деморежим');
                                                } else {
                                                    $('#cash-box-panel').addClass('bg-blue');
                                                    $('#cash-box-status').text('работает');
                                                }
                                            }
                                        }
                                    }
