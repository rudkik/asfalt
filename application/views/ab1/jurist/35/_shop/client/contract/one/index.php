...
<tr>
    ...
    <td>
        <input <?php if ($data->values['is_fixed_contract'] == 1) { echo ' value="1" checked'; }else{echo 'value="0"';} ?> type="checkbox" class="minimal" data-id="<?php echo $data->id; ?>">
    </td>
    ...
    <td <?php if($data->values['client_contract_type_id'] < 1){ ?>class="tr-red"<?php } ?>><?php echo $data->getElementValue('client_contract_type_id'); ?></td>
    <td <?php if(empty($data->values['subject'])){ ?>class="tr-red"<?php } ?>><?php echo $data->values['subject']; ?></td>
    <td <?php if($data->values['client_contract_status_id'] < 1){ ?>class="tr-red"<?php } ?>><?php echo $data->getElementValue('client_contract_status_id'); ?></td>
    <td <?php if($data->values['executor_shop_worker_id'] < 1 && $data->values['client_contract_type_id'] != Model_Ab1_ClientContract_Type::CLIENT_CONTRACT_TYPE_SALE_PRODUCT){ ?>class="tr-red"<?php } ?>>
        <?php
        if($data->values['executor_shop_worker_id'] > 0){
            echo $data->getElementValue('executor_shop_worker_id');
        }
        ?>
    </td>
    ...
    <td>
        <ul class="list-inline tr-button <?php if ($data->values['is_delete'] == 1) { echo ' un-'; } ?>delete">
            ...
            $button = 'Изменить';
            $isDelete = true;
            if(false && $data->values['client_contract_type_id'] > 0 && $data->values['client_contract_type_id'] <= 100 && !$siteData->operation->getIsAdmin()){
                $fixed['is_show'] = 1;
                $button = 'Просмотр';
                $isDelete = false;
            }
