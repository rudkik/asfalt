	...
	<div class="nav-tabs-custom nav-tabs-navy">
        ...
        <div class="tab-content">
            <div class="tab-pane active">
                ...
                <?php
                $view = View::factory('ab1/bookkeeping/35/main/_shop/invoice/filter');
                $view->siteData = $siteData;
                $view->data = $data;
                echo Helpers_View::viewToStr($view);
                ?>
            ...
            <div class="nav-tabs-custom" style="margin-bottom: 0px;">
                <ul id="tab-status" class="nav nav-tabs pull-right ui-sortable-handle">
                    ...
                    <?php } ?>
                    <?php if(Request_RequestParams::getParamInt('shop_client_id') > 0){ ?>
                    <li class="pull-left" style="margin-right: 10px;">
                        <button onclick="getClientInvoice();" class="btn bg-purple btn-flat">Документы клиенту</button>
                    </li>
                    <?php } ?>
...
<script>
    function getClientInvoice() {
        $('#modal-client-invoice').remove();
        jQuery.ajax({
            url: '<?php echo Func::getFullURL($siteData, '/shopclient/invoice', array(), array_merge($_POST, $_GET)); ?>',
            data: ({}),
            type: "GET",
            success: function (data) {
                $('body').append(data);
                $('#modal-client-invoice').modal('show');
            },
            error: function (data) {
