<div class="row">
    ...
    $n = 2;
    $i = 1;
    foreach ($data['view::View_Shop_Good\-catalogs-produktciya']->childs as $value){
        if($i == $n + 1){
...
<?php if($siteData->pages > 1) { ?>
    <ul class="box-pagination">
        ...
        unset($urlParams['rubric']);
        $urlParams['page'] = '_page_';
        $url = $siteData->urlBasic . '/catalogs' . $siteData->urlSEO . URL::query($urlParams, FALSE);
        $url = str_replace('page=_page_', 'page=$page$', $url);

        // $page$ - в место него будет вставлен номер страницы
        // $url$ - в место ссылки

        // предыдущая страница
        $previous_active = '<li class="prev"><a href="$url$"></a></li>';
        $previous_not_active = '';

        // следующая страница
        $next_active = '<li class="next"><a href="$url$"></a></li>';
        $next_not_active = '';
