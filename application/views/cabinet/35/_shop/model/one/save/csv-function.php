...
function isIgnore($name){
    ...
    switch($name){
        ...
        case 'good_id': return TRUE; break;
...
function writeElementName(&$data, $name, $value){
    switch($name){
        ...
        case 'good_select_type_id': $result = Arr::path($data->values, Model_Basic_BasicObject::FIELD_ELEMENTS. '.good_select_type_id.name', ''); break;
        case 'shop_table_unit_type_id': $result = Arr::path($data->values, Model_Basic_BasicObject::FIELD_ELEMENTS. '.shop_table_unit_type_id.name', ''); break;
        default:
            $result = $value;
    }
