...
<tr>
    ...
    <td><img class="table-preview" data-action="modal-image" data-id="<?php echo $data->id; ?>" data-href="<?php echo Func::getFullURL($siteData, '/shopmodel/addimg');?>" src="<?php echo Helpers_Image::getPhotoPath($data->values['image_path'], 68, 52); ?>"></td>
    <?php } ?>
    <td>
...
<?php
$count = 4;
if (Func::isShopMenu('shopmodel/image?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}
if(empty($editFields) || (in_array('article', $editFields))){$count++;}
if(empty($editFields) || (in_array('shop_table_rubric_id', $editFields))){if (Func::isShopMenu('shopmodel/rubric?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}}
?>
<?php if($count > 0) { ?>
        <tr>
