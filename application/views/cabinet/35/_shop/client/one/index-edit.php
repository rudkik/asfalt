...
<tr>
    <td>
        <input name="shop_clients[<?php echo $data->id; ?>][is_public]" <?php if ($data->values['is_public'] == 1) { echo ' value="1" checked'; }else{echo 'value="0"';} ?> type="checkbox" class="minimal is-public">
    </td>
    <td><?php echo $data->id; ?></td>
    <?php if (Func::isShopMenu('shopclient/image?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
    <td><img class="table-preview" data-action="modal-image" data-id="<?php echo $data->id; ?>" data-href="<?php echo Func::getFullURL($siteData, '/shopclient/addimg');?>" src="<?php echo Helpers_Image::getPhotoPath($data->values['image_path'], 68, 52); ?>"></td>
    <?php } ?>
    ...
    <?php if (Func::isShopMenu('shopclient/rubric?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
        <td>
            <div class="input-group">
                <select name="shop_clients[<?php echo $data->id; ?>][shop_table_rubric_id]" class="form-control select2" style="width: 100%;">
    ...
    <?php } ?>
    <td>
        <input name="shop_clients[<?php echo $data->id; ?>][name]" type="text" class="form-control" placeholder="<?php echo SitePageData::setPathReplace('type.form_data.shop_client.fields_title.name', SitePageData::CASE_FIRST_LETTER_UPPER); ?>" value="<?php echo htmlspecialchars($data->values['name'], ENT_QUOTES);?>">
    </td>
    <?php if(empty($editFields) || (in_array('last_name', $editFields))){ ?>
        <td>
            <input name="shop_clients[<?php echo $data->id; ?>][last_name]" type="text" class="form-control" placeholder="Артикул" value="<?php echo htmlspecialchars($data->values['last_name'], ENT_QUOTES);?>">
        </td>
    <?php } ?>
    <?php if(empty($editFields) || (in_array('email', $editFields))){ ?>
        <td>
            <input name="shop_clients[<?php echo $data->id; ?>][email]" type="text" class="form-control" placeholder="Артикул" value="<?php echo htmlspecialchars($data->values['email'], ENT_QUOTES);?>">
        </td>
    <?php } ?>
...
<?php
$count = 4;
if (Func::isShopMenu('shopclient/image?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}
if(empty($editFields) || (in_array('email', $editFields))){$count++;}
if(empty($editFields) || (in_array('shop_table_rubric_id', $editFields))){if (Func::isShopMenu('shopclient/rubric?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}}
?>
<?php if($count > 0) { ?>
        <tr>
                    ...
                    <?php if (in_array('old_id', $editFields)) { ?>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input name="shop_clients[<?php echo $data->id; ?>][old_id]" class="form-control"
                            ...
                            if((key_exists($field, $options)) || (in_array('options.'.$field, $editFields))) {
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input
                                            name="shop_clients[<?php echo $data->id; ?>][options][<?php echo $field; ?>]"
                            ...
                            if (in_array($field, $editFields)) {
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input
                                            name="shop_clients[<?php echo $data->id; ?>][options][<?php echo $objOption['field']; ?>]"
                    ...
                    <?php if (in_array('info', $editFields)) { ?>
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea name="shop_clients[<?php echo $data->id; ?>][info]" placeholder="Описание..."
