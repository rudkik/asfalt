<form action="<?php echo Func::getFullURL($siteData, '/shopfile/savesort');?>" method="post">
	...
	<table class="table table-hover table-db">
		<tr>
			...
			<?php if (Func::isShopMenu('shopfile/article?type='.Request_RequestParams::getParamInt('type'), $siteData)){ ?>
				<th class="tr-header-price"><?php echo SitePageData::setPathReplace('type.form_data.shop_file.fields_title.article', SitePageData::CASE_FIRST_LETTER_UPPER); ?></th>
			<?php } ?>
			...
			<?php } ?>
			<th><?php echo SitePageData::setPathReplace('type.form_data.shop_file.fields_title.name', SitePageData::CASE_FIRST_LETTER_UPPER); ?></th>
			<th class="tr-header-buttom"></th>
		</tr>
		<tbody id="sort-record">
		<?php
		foreach ($data['view::_shop/file/one/sort']->childs as $value) {
			echo $value->str;
		}
		?>
		</tbody>
