<form action="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoptableunit/savelist" method="post">
    ...
    <table id="edit-list" class="table table-hover table-db">
        <tr>
            ...
            <?php if(empty($editFields) || (in_array('shop_table_rubric_id', $editFields))){ ?>
                <?php if ((Func::isShopMenu('shoptableunit/rubric?type='.Request_RequestParams::getParamInt('type'), $siteData))){ ?>
                    <th class="tr-header-select"><?php echo SitePageData::setPathReplace('type.form_data.shop_table_unit.fields_title.rubric', SitePageData::CASE_FIRST_LETTER_UPPER); ?></th>
                <?php } ?>
            <?php }?>
            <th>
        ...
        <?php
        foreach ($data['view::_shop/_table/unit/one/index-edit']->childs as $value) {
            echo $value->str;
        }
        ?>
    </table>
