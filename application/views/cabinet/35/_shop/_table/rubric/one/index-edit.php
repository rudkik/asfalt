...
$objOptions = Arr::path($data->values, Model_Basic_BasicObject::FIELD_ELEMENTS.'.shop_table_catalog_id.fields_options.shop_table_hashtag_rubric', array());
...
if ((Func::isShopMenu('shoptablerubric/text-html?type='.$data->values['shop_table_catalog_id'], $siteData))
|| (Func::isShopMenu('shoptablerubric/text?type='.$data->values['shop_table_catalog_id'], $siteData))) {
...
<tr>
    <td>
        <input name="shop_table_rubrics[<?php echo $data->id; ?>][is_public]" <?php if ($data->values['is_public'] == 1) { echo ' value="1" checked'; }else{echo 'value="0"';} ?> type="checkbox" class="minimal is-public">
    </td>
    <td><?php echo $data->id; ?></td>
    <?php if (Func::isShopMenu('shoptablerubric/image?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
        <td><img class="table-preview" data-action="modal-image" data-id="<?php echo $data->id; ?>" data-href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoptablerubric/addimg" src="<?php echo Helpers_Image::getPhotoPath($data->values['image_path'], 68, 52); ?>"></td>
    <?php } ?>
        ...
        <?php if (Func::isShopMenu('shoptablerubric/root?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
            <td>
                <div class="input-group">
                    <select name="shop_table_rubrics[<?php echo $data->id; ?>][root_id]" class="form-control select2" style="width: 100%;">
    ...
    <?php } ?>
    <td>
        <input name="shop_table_rubrics[<?php echo $data->id; ?>][name]" type="text" class="form-control" placeholder="<?php echo SitePageData::setPathReplace('type.form_data.shop_table_rubric.fields_title.name', SitePageData::CASE_FIRST_LETTER_UPPER); ?>" value="<?php echo htmlspecialchars($data->values['name'], ENT_QUOTES);?>">
    </td>
    <td <?php if($countFields > 0){ echo 'rowspan="2"';}?>>
        <ul class="list-inline tr-button <?php if ($data->values['is_delete'] == 1) { echo ' un-'; } ?>delete">
            <li><a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoptablerubric/edit?id=<?php echo $data->id; ?>&type=<?php echo $data->values['shop_table_catalog_id']; ?><?php if($siteData->superUserID > 0){echo '&shop_id='.$siteData->shopID;} ?><?php if($siteData->branchID > 0){echo '&shop_branch_id='.$siteData->branchID;} ?>" class="link-blue text-sm"><i class="fa fa-edit margin-r-5"></i> Изменить</a></li>
            <li><a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoptablerubric/clone?id=<?php echo $data->id; ?>&type=<?php echo $data->values['shop_table_catalog_id']; ?><?php if($siteData->superUserID > 0){echo '&shop_id='.$siteData->shopID;} ?><?php if($siteData->branchID > 0){echo '&shop_branch_id='.$siteData->branchID;} ?>" class="link-black text-sm"><i class="fa fa-clone margin-r-5"></i> Дублировать</a></li>
...
<?php if($countFields > 0){?>
    ...
    if (Func::isShopMenu('shoptablerubric/image?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}
    if(empty($editFields) || (in_array('root_id', $editFields))){if (Func::isShopMenu('shoptablerubric/root?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}}
            ...
            <td colspan="<?php echo $count; ?>">
                <div class="row"style="margin-left: -8px;margin-right: -8px;">
                    <?php if (in_array('old_id', $editFields)) { ?>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input name="shop_table_rubrics[<?php echo $data->id; ?>][old_id]" class="form-control"
                    ...
                    if (in_array('options', $editFields)) {
                        $objOptions = Arr::path($data->values, Model_Basic_BasicObject::FIELD_ELEMENTS . '.shop_table_catalog_id.fields_options.shop_table_rubric', array());
                        if (!is_array($objOptions)) {
                            $objOptions = array();
                        }
                            ...
                            if((key_exists($field, $options)) || (in_array('options.'.$field, $editFields))) {
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input
                                            name="shop_table_rubrics[<?php echo $data->id; ?>][options][<?php echo $field; ?>]"
                            ...
                            if (in_array($field, $editFields)) {
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input
                                            name="shop_table_rubrics[<?php echo $data->id; ?>][options][<?php echo $objOption['field']; ?>]"
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="text">Описание</label>
                                    <textarea name="shop_table_rubrics[<?php echo $data->id; ?>][text]" placeholder="<?php echo SitePageData::setPathReplace('type.form_data.shop_table_rubric.fields_title.text', SitePageData::CASE_FIRST_LETTER_UPPER); ?>"
