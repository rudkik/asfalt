<table class="table table-hover table-db">
	<tr>
		...
		<?php if ((Func::isShopMenu('shoptablehashtag/rubric?type='.Request_RequestParams::getParamInt('type'), $siteData))){ ?>
			<th class="tr-header-rubric"><?php echo SitePageData::setPathReplace('type.form_data.shop_table_hashtag.fields_title.rubric', SitePageData::CASE_FIRST_LETTER_UPPER); ?></th>
		<?php } ?>
		<th>
		...
		<th  class="tr-header-sort">
			<a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoptablehashtag/index<?php echo Func::getAddURLSortBy($siteData->urlParams, 'order'); ?>" class="link-black">Сортировка</a>
			<a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoptablehashtag/index<?php echo Func::getAddURLSortBy($siteData->urlParams, 'order'); ?>" class="link-blue">
	...
	<?php
	foreach ($data['view::_shop/_table/hashtag/one/index']->childs as $value) {
		echo $value->str;
	}
	?>
