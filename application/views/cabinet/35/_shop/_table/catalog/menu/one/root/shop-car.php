<?php if ((Func::isShopMenu('shopcar/index?type='.$data->id, $siteData))){ ?>
    <li <?php if(!(($siteData->controllerName == 'shopcar')
        && (Request_RequestParams::getParamInt('type') == $data->id))){echo 'class="menu-left"';} ?>>
        <a href="<?php echo Func::getFullURL($siteData, '/shopcar/index', array(), array('is_public_ignore' => 1, 'type' => $data->id)); ?>"><i class="fa fa-circle-o text-light-blue"></i>
...
<?php if ((Func::isShopMenu('shoptablerubric/index?type='.$data->id, $siteData))){ ?>
    <li <?php if(!(($siteData->controllerName == 'shoptablerubric')
        ...
        && (Request_RequestParams::getParamInt('table_id') == Model_Shop_Car::TABLE_ID))){echo 'class="menu-left"';} ?>>
        <a href="<?php echo Func::getFullURL($siteData, '/shoptablerubric/index', array(), array('is_public_ignore' => 1, 'type' => $data->id, 'table_id' => Model_Shop_Car::TABLE_ID)); ?>"><i class="fa fa-circle-o text-light-blue"></i>
