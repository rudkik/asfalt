<form action="<?php echo Func::getFullURL($siteData, '/shopcar/savesort');?>" method="post">
	...
	<table class="table table-hover table-db">
		<tr>
			...
			<?php } ?>
			<?php if (Func::isShopMenu('shopcar/article?type='.Request_RequestParams::getParamInt('type'), $siteData)){ ?>
				<th class="tr-header-price"><?php echo SitePageData::setPathReplace('type.form_data.shop_car.fields_title.article', SitePageData::CASE_FIRST_LETTER_UPPER); ?></th>
			<?php } ?>
			...
			<?php } ?>
			<th><?php echo SitePageData::setPathReplace('type.form_data.shop_car.fields_title.name', SitePageData::CASE_FIRST_LETTER_UPPER); ?></th>
		...
		<?php
		foreach ($data['view::_shop/car/one/sort']->childs as $value) {
			echo $value->str;
		}
		?>
		</tbody>
