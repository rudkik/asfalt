...
<table id="panel-cars" data-id="<?php echo count($data['view::_shop/car/one/similar']->childs); ?>" class="table table-hover table-db margin-b-5">
    <tr>
    ...
    <?php
    foreach ($data['view::_shop/car/one/similar']->childs as $value){
        echo $value->str;
    }
    ?>
</table>
<?php if (count($data['view::_shop/car/one/similar']->childs) == 0){ ?>
    <div id="div-not-options-similar-cars" class="contacts-list-msg text-center margin-b-5">Значения не заданы</div>
<?php }?>
<div class="text-right" data-toggle="modal" data-target="#find-similar">
    <button type="button" class="btn btn-warning" onclick="actionAddTR('body_panel_catalog', 'tr_panel_catalog', 'div-not-options-similar-cars')"><i class="fa fa-fw fa-plus"></i> Добавить</button>
</div>