<tr>
    <td>
        <input name="set-is-public" <?php if ($data->values['is_public'] == 1) { echo ' value="1" checked'; }else{echo 'value="0"';} ?> href="<?php echo Func::getFullURL($siteData, '/shoppersondiscount/edit', array('id' => 'id'), array(), $data->values); ?>" type="checkbox" class="minimal">
    </td>
    ...
    <?php } ?>
    <?php if (Func::isShopMenu('shop_person_discount/article?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
    ...
    <?php if (Func::isShopMenu('shop_person_discount/price?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
        <td>
            <?php
