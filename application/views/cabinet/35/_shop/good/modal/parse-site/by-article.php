<div id="parse-site-good" class="modal">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group input-group-select">
                        <select class="form-control select2" style="width: 100%;" name="site">
                            ...
                            <option data-id="ekt.kz" value="ekt.kz">Ekt.kz (русская версия) поиск по артиклу. Дата обновления: 14.02.2019</option>
                            ...
                            <option data-id="nzeta.ru" value="nzeta.ru">Nzeta.ru (русская версия) поиск по артиклу. Дата обновления: 07.04.2019</option>
                            <option data-id="lotki.ru" value="lotki.ru">Lotki.ru (русская версия) поиск по артиклу. Дата обновления: 26.04.2019</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
    ...
    <script>
        ...
        function parseSite() {
            if ($('#parse-site-good').attr('start') != 1){
                return FALSE;
            }
            ...
            var site = $('#parse-site-good [name="site"]').val();
            var isReplace = $('#parse-site-good [name="is_replace"]').val();

            jQuery.ajax({
                url: '/cabinet/shopgood/parse_site_by_article',
                data: ({
                    'id': (id),
                    'site': (site),
                ...
                success: function (data) {
                    $('#parse-site-good [data-id="progress"]').attr('style', 'width:'+(100 / count * index)+'%');
                    $('#parse-site-good [data-id="progress-status"]').text('Обработано ' + index + ' записей(ь)');
                    parseSite();
                },
                error: function (data) {
                    console.log(data.responseText);
                    $('#parse-site-good [data-id="button-parse"]').text('Продолжить парсинг');
                }
            });
        }

        function parseSiteStart() {
            $('#parse-site-good').attr('start', 1);
            $('#parse-site-good [data-id="button-parse"]').text('Идет парсинг');
            parseSite();
        }
        function parseSiteStop() {
            $('#parse-site-good').attr('start', 0);
            $('#parse-site-good').modal('hide');
        }
    </script>
</div>