...
if(in_array('shop_table_unit_id', $editFields)){$countFields++;}
if(in_array('shop_table_brand_id', $editFields)){$countFields++;}
...
<tr>
    ...
    <td><img class="table-preview" data-action="modal-image" data-id="<?php echo $data->id; ?>" data-href="<?php echo Func::getFullURL($siteData, '/shopgood/addimg');?>" src="<?php echo Helpers_Image::getPhotoPath($data->values['image_path'], 68, 52); ?>"></td>
    <?php } ?>
    <?php if(empty($editFields) || (in_array('article', $editFields))){ ?>
    <td>
        <input name="shop_goods[<?php echo $data->id; ?>][article]" type="text" class="form-control" placeholder="Артикул" value="<?php echo htmlspecialchars($data->values['article'], ENT_QUOTES);?>">
    </td>
    <?php } ?>
    ...
    <?php if (Func::isShopMenu('shopgood/rubric?type='.$data->values['shop_table_catalog_id'], $siteData)){ ?>
        <td>
            <div class="input-group">
                <select name="shop_goods[<?php echo $data->id; ?>][shop_table_rubric_id]" class="form-control select2" style="width: 100%;">
    ...
    <td <?php if($countFields > 0){ echo 'rowspan="2"';}?>>
        <ul class="list-inline tr-button <?php if ($data->values['is_delete'] == 1) { echo ' un-'; } ?>delete">
            <li><a href="<?php echo Func::getFullURL($siteData, '/shopgood/edit', array('id' => 'id', 'type' => 'shop_table_catalog_id'), array(), $data->values); ?>" class="link-blue text-sm"><i class="fa fa-edit margin-r-5"></i> Изменить</a></li>
...
<?php
$count = 4;
if (Func::isShopMenu('shopgood/image?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}
if(empty($editFields) || (in_array('article', $editFields))){$count++;}
if(empty($editFields) || (in_array('shop_table_rubric_id', $editFields))){if (Func::isShopMenu('shopgood/rubric?type='.$data->values['shop_table_catalog_id'], $siteData)){$count++;}}
?>
<?php if($count > 0) { ?>
        <tr>
            <td colspan="<?php echo $count; ?>">
                <div class="row"style="margin-left: -8px;margin-right: -8px;">
                    <?php if (in_array('shop_table_brand_id', $editFields)) { ?>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="shop_table_brand_id"><?php echo SitePageData::setPathReplace('type.form_data.shop_good.fields_title.brand', SitePageData::CASE_FIRST_LETTER_UPPER); ?></label>
                                <div class="input-group">
                                    <select name="shop_goods[<?php echo $data->id; ?>][shop_table_brand_id]" class="form-control select2" style="width: 100%;">
                                        <option value="0" data-id="0">Без значения</option>
                                        <?php
                                        $s = 'data-id="'.$data->values['shop_table_brand_id'].'"';
                                        echo trim(str_replace($s, $s.' selected',$siteData->replaceDatas['view::_shop/_table/brand/list/list']));
                                        ?>
                                    </select>
                                    <span class="input-group-btn"><button data-action="copy-select" class="btn btn-info btn-flat" type="button"><i class="fa fa-fw fa-copy"></i></button></span>
                                    <span class="input-group-btn"><button data-action="insert-select" class="btn btn-info btn-flat" type="button"><i class="fa fa-fw fa-paste"></i></button></span>
                    ...
                    <?php } ?>
                    <?php if (in_array('shop_table_unit_id', $editFields)) { ?>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="shop_table_unit_id"><?php echo SitePageData::setPathReplace('type.form_data.shop_good.fields_title.unit', SitePageData::CASE_FIRST_LETTER_UPPER); ?></label>
                                <div class="input-group">
                                    <select name="shop_goods[<?php echo $data->id; ?>][shop_table_unit_id]" class="form-control select2" style="width: 100%;">
                                        <option value="0" data-id="0">Без значения</option>
                                        <?php
                                        $s = 'data-id="'.$data->values['shop_table_unit_id'].'"';
                                        echo trim(str_replace($s, $s.' selected',$siteData->replaceDatas['view::_shop/_table/unit/list/list']));
                                        ?>
                                    </select>
                                    <span class="input-group-btn"><button data-action="copy-select" class="btn btn-info btn-flat" type="button"><i class="fa fa-fw fa-copy"></i></button></span>
                                    <span class="input-group-btn"><button data-action="insert-select" class="btn btn-info btn-flat" type="button"><i class="fa fa-fw fa-paste"></i></button></span>
