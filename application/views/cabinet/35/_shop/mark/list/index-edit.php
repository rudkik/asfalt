<form action="<?php echo Func::getFullURL($siteData, '/shopmark/savelist'); ?>" method="post">
...
<table id="edit-list" class="table table-hover table-db">
    <tr>
        ...
        <th class="tr-header-id">
            <a href="<?php echo Func::getFullURL($siteData, '/shopmark/index'). Func::getAddURLSortBy($siteData->urlParams, 'id'); ?>" class="link-black">ID</a>
            <a href="<?php echo Func::getFullURL($siteData, '/shopmark/index'). Func::getAddURLSortBy($siteData->urlParams, 'id'); ?>" class="link-blue">
                <i class="fa fa-fw fa-sort-numeric-<?php if (Arr::path($siteData->urlParams, 'sort_by.id', '') === 'asc'){echo 'desc';}else{echo 'asc';}?>"></i>
            </a>
        </th>
        <?php if ((Func::isShopMenu('shopmark/image?type='.Request_RequestParams::getParamInt('type'), array(), $siteData))){ ?>
        <th class="tr-header-photo">Фото</th>
        <?php }?>
        <th>
            <a href="<?php echo Func::getFullURL($siteData, '/shopmark/index'). Func::getAddURLSortBy($siteData->urlParams, 'name'); ?>" class="link-black">Название</a>
            <a href="<?php echo Func::getFullURL($siteData, '/shopmark/index'). Func::getAddURLSortBy($siteData->urlParams, 'name'); ?>" class="link-blue">
    ...
    <?php
    foreach ($data['view::_shop/mark/one/index-edit']->childs as $value) {
        echo $value->str;
    }
    ?>
</table>
