<form action="<?php echo Func::getFullURL($siteData, '/shopaddresscontact/savelist'); ?>" method="post">
...
<table id="edit-list" class="table table-hover table-db">
    <tr>
        ...
        <?php if(empty($editFields) || (in_array('city', $editFields))){ ?>
            <?php if ((Func::isShopMenu('shopaddresscontact/city', $siteData))){ ?>
                <th class="tr-header-rubric">Страна/город</th>
            <?php }?>
        <?php }?>
        <?php if(empty($editFields) || (in_array('shop_table_rubric_id', $editFields))){ ?>
            ...
            <?php } ?>
        <?php }?>
        <?php if(empty($editFields) || (in_array('contact_type_id', $editFields))){ ?>
            <th class="tr-header-rubric">Вид контакта</th>
        <?php }?>
        <th>
            <a href="<?php echo Func::getFullURL($siteData, '/shopaddresscontact/index'). Func::getAddURLSortBy($siteData->urlParams, 'name'); ?>" class="link-black"><?php echo SitePageData::setPathReplace('type.form_data.shop_addresscontact.fields_title.name', SitePageData::CASE_FIRST_LETTER_UPPER); ?></a>
    ...
    <?php
    foreach ($data['view::_shop/address/contact/one/index-edit']->childs as $value) {
        echo $value->str;
    }
    ?>
</table>
