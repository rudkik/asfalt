...
if((!empty($imageTypes)) && is_array($imageTypes)){
   foreach ($imageTypes as $imageType){
       $selectImageTypes .= '<option data-id="'.$imageType['field'].'" value="'.$imageType['field'].'">'.$imageType['title'].'</option>';
   }
}
?>
...
<?php } ?>
<textarea id="text-source" rows="5" class="form-control" style="margin-bottom: 20px"></textarea>
<button type="button" class="btn btn-warning pull-left" onclick="InsertText()"><i class="fa fa-fw fa-plus"></i> Встать в текст</button>
<button type="submit" class="btn btn-primary pull-right">Сохранить</button>

<script>
    function InsertText() {
        CKEDITOR.instances.text.setData($('#text-source').val().replace(/(?:\r\n|\r|\n)/g, '<br />'));
    }
</script>