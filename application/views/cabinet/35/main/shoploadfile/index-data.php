		...
		<div class="col-md-12">
			<div class="nav-tabs-custom" style="margin-bottom: 0px;">
				<ul class="nav nav-tabs pull-right ui-sortable-handle">
					<li class="<?php if(Request_RequestParams::getParamBoolean('is_old') === TRUE){echo 'active';}?>"><a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoploadfile/index_data?id=<?php echo Request_RequestParams::getParamInt('id'); ?>&is_old=1<?php if($siteData->branchID > 0){echo '&shop_branch_id='.$siteData->branchID;} ?><?php if($siteData->superUserID > 0){echo '&shop_id='.$siteData->shopID;} ?>" >Старые</a></li>
					<li class="<?php if(Request_RequestParams::getParamBoolean('is_new') === TRUE){echo 'active';}?>"><a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoploadfile/index_data?id=<?php echo Request_RequestParams::getParamInt('id'); ?>&is_new=1<?php if($siteData->branchID > 0){echo '&shop_branch_id='.$siteData->branchID;} ?><?php if($siteData->superUserID > 0){echo '&shop_id='.$siteData->shopID;} ?>" >Новые</a></li>
					<li class="<?php if((Request_RequestParams::getParamBoolean('is_new') !== TRUE) && (Request_RequestParams::getParamBoolean('is_old') !== TRUE)){echo 'active';}?>"><a href="<?php echo $siteData->urlBasic; ?>/<?php echo $siteData->actionURLName; ?>/shoploadfile/index_data?id=<?php echo Request_RequestParams::getParamInt('id'); ?><?php if($siteData->branchID > 0){echo '&shop_branch_id='.$siteData->branchID;} ?><?php if($siteData->superUserID > 0){echo '&shop_id='.$siteData->shopID;} ?>" >Все <i class="fa fa-fw fa-info text-blue"></i></a></li>
				</ul>
			</div>
			<div class="box box-primary padding-t-5">
				<div class="box-body table-responsive no-padding">
					<?php echo trim($data['view::shoploadfile/index-data']); ?>
				</div>
			</div>
		</div>
	</div>
