...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($dest[0] != 'F') {
			$name = mb_ereg_replace ('/[\s]+/', '_', $name);
			$name = mb_ereg_replace ('/[^a-zA-Z0-9А-Яа-я_\.-]/', '', $name);
		}
		if ($this->sign) {
		...
		switch($dest) {
			...
			case 'D': {
				...
				if (strpos(php_sapi_name(), 'cgi') === false) {
					...
					header('Content-Type: application/octet-stream;charset=UTF-8', false);
					header('Content-Type: application/download;charset=UTF-8', false);
