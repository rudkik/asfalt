...
class Helpers_File_UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => DOCROOT.'tmp_files'. DIRECTORY_SEPARATOR,
            ...
            'image_versions' => array(
                ...
                */
                'thumbnail' => array(
                    ...
                    'max_width' => 99, // either specify width, or set to 0. Then width is automatically adjusted - keeping aspect ratio to a specified max_height.
                    'max_height' => 99 // either specify height, or set to 0. Then height is automatically adjusted - keeping aspect ratio to a specified max_width.
                )
            ),
            'print_response' => true
        );
    ...
    protected function get_full_url() {
        ...
        return
            ...
            substr($s,0, strrpos($s, '/'));
    }

    protected function get_file_url() {
        $https = !empty($_SERVER['HTTPS']) && strcasecmp($_SERVER['HTTPS'], 'on') === 0 ||
            !empty($_SERVER['HTTP_X_FORWARDED_PROTO']) &&
            strcasecmp($_SERVER['HTTP_X_FORWARDED_PROTO'], 'https') === 0;
        ...
        return
            ...
            (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ($_SERVER['SERVER_NAME'].
                ($https && $_SERVER['SERVER_PORT'] === 443 ||
                $_SERVER['SERVER_PORT'] === 80 ? '' : ':'.$_SERVER['SERVER_PORT']))).'/tmp_files/';
    }
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
            ...
            if(!empty($file->thumbnailUrl)) {
                $fileInfo = pathinfo($file->thumbnailUrl);
                $file->thumbnailUrl = $fileInfo['dirname'] . '/' . $fileInfo['filename']
                    . '-' . $this->options['image_versions']['thumbnail']['max_width']
                    . 'x' . $this->options['image_versions']['thumbnail']['max_height']
                    . '.' . $fileInfo['extension'];
            }
            return $file;
        }
     ...
     * @return mixed|string
     */
    protected function get_in_translate_to_en($string, $isOnlySymbols = FALSE)
    {
    ...
    protected function get_file_name($file_path, $name, $size, $type, $error,
        ...
        $name = $this->trim_file_name($file_path, $name, $size, $type, $error,
            $index, $content_range);
        $name = $this->get_in_translate_to_en($name);
