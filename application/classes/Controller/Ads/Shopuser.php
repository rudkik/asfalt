...
class Controller_Ads_Shopuser extends Controller_Ads_BasicAds {
    ...
    public function action_auth() {
        $this->_sitePageData->url = '/ads/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/ads/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/ads/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            $this->redirect('/ads/shopparcel/index');
        }
    }
    ...
    public function action_registration() {
        ...
        $this->_sitePageData->url = '/ads/shopuser/registration';
        ...
        // генерируем основную часть
        $view = View::factory('ads/' . $this->_sitePageData->languageID . '/main/shopuser-registration');
        $view->data = array();
        $view->siteData = $this->_sitePageData;
        $result = Helpers_View::viewToStr($view);
        ...
        $view = View::factory('ads/' . $this->_sitePageData->languageID . '/index');
        $view->data = array();
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/ads");
    }

    public function action_resetpassword() {
        $this->_sitePageData->url = '/ads/shopuser/resetpassword';
        ...
        if (mail(!$email, 'Reset password', 'OTO Ads')) {
