...
class Controller_Cabinet_BasicShop extends Controller_Cabinet_BasicList {
	...
	protected function _readShopInterface()
	{
		...
		if(! $this->getDBObject($model, $shopID)){
            throw new HTTP_Exception_404('Shop database not found.');
		}
	...
	protected function _readUserInterface()
	{
		...
		if (!($auth->isLoginSessionShop())){
			...
			$email = Arr::path($arr, 'auth.email', Arr::path($arr, 'auth_email', ''));
			$password = Arr::path($arr, 'auth.password', Arr::path($arr, 'auth_password', ''));

            if((empty($email)) || (empty($password))){
                $this->redirect('/cabinet?error=1');
			}else {
                if (!$auth->login($email, $password)) {
                    $this->redirect('/cabinet?error=2');
                }else{
