...
class Controller_Cabinet_ShopFile extends Controller_Cabinet_File
{
	public function __construct(Request $request, Response $response)
	{
        $this->dbObject = 'DB_Shop_File';
		$this->controllerName = 'shopfile';
		$this->tableID = Model_Shop_File::TABLE_ID;
		$this->tableName = Model_Shop_File::TABLE_NAME;
	...
	public function action_index() {
		$this->_sitePageData->url = '/cabinet/shopfile/index';
		...
		// получаем список
        View_View::find('DB_Shop_File', $this->_sitePageData->shopID, "_shop/file/list/index", "_shop/file/one/index",
			$this->_sitePageData, $this->_driverDB, array('limit_page' => 25), array('shop_table_rubric_id'));

		$this->_putInMain('/main/_shop/file/index');
	}

	public function action_sort(){
		$this->_sitePageData->url = '/cabinet/shopfile/sort';
		...
		// получаем список
		$this->_sitePageData->replaceDatas['view::_shop/file/list/index'] = View_View::find('DB_Shop_File', $this->_sitePageData->shopID,
			"_shop/file/list/sort", "_shop/file/one/sort",
	...
	public function action_index_edit() {
		$this->_sitePageData->url = '/cabinet/shopfile/index_edit';
		...
		$arr = Arr::path($type['fields_options'], 'shop_file', array());
		...
		// получаем список
		$this->_sitePageData->replaceDatas['view::_shop/file/list/index'] = View_View::find('DB_Shop_File', $this->_sitePageData->shopID,
			"_shop/file/list/index-edit", "_shop/file/one/index-edit",
			$this->_sitePageData, $this->_driverDB,
	...
	{
		$this->_sitePageData->url = '/cabinet/shopfile/new';

		// задаем данные, которые будут меняться
		$this->_setGlobalDatas(
			array(
				'view::_shop/file/one/new',
			)
		);
		...
		$this->_sitePageData->replaceDatas['view::_shop/file/one/new'] = Helpers_View::getViewObject($dataID, new Model_Shop_File(),
			'_shop/file/one/new', $this->_sitePageData, $this->_driverDB);

		$this->_putInMain('/main/_shop/file/new');
	}

	public function action_edit()
	{
		$this->_sitePageData->url = '/cabinet/shopfile/edit';

		// задаем данные, которые будут меняться
		$this->_setGlobalDatas(
			array(
				'view::_shop/file/one/edit',
			)
		);

		// id записи
		$shopFileID = Request_RequestParams::getParamInt('id');
		if ($shopFileID === NULL) {
			throw new HTTP_Exception_404('Files not is found!');
		}else {
			$modelFile = new Model_Shop_File();
			if (! $this->dublicateObjectLanguage($modelFile, $shopFileID)) {
				throw new HTTP_Exception_404('Files not is found!');
			}
		}

		// тип объекта
		$type = $this->_getType($modelFile->getShopTableCatalogID());

		$this->_requestTableObject($type, $modelFile);
		...
		// получаем данные
        View_View::findOne('DB_Shop_File', $this->_sitePageData->shopID, "_shop/file/one/edit",
			$this->_sitePageData, $this->_driverDB, array('id' => $shopFileID), array('shop_table_catalog_id'));

		$this->_putInMain('/main/_shop/file/edit');
	}

	public function action_save()
	{
		$this->_sitePageData->url = '/cabinet/shopfile/save';
		$result = Api_Shop_File::save($this->_sitePageData, $this->_driverDB);
