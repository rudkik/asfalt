...
class Controller_Nur_BasicShop extends Controller_BasicAdmin {
    ...
    protected function _readUserInterface()
    {
        ...
        if (!($auth->isLoginSessionShop())){
            ...
            if((empty($email)) || (empty($password))){
                $this->redirect('/'.$this->_sitePageData->actionURLName);
            }else {
        ...
        switch ($operationType){
            case Model_Nur_Shop_Operation::RUBRIC_BOOKKEEPING:
                if(($operationType > 0) && ($this->_sitePageData->actionURLName != 'nur-bookkeeping')){
                    $this->redirect('/'.$this->_sitePageData->actionURLName);
                }
                break;
        }
    ...
    public function before()
    {
        ...
        $this->_sitePageData->shopShablonPath = 'nur/'.$this->prefixView;
	...
	protected function _readShopInterface()
	{
		...
		if ($shopID < 1){
			throw new HTTP_Exception_404('Shop ('.$shopID.') not found.');
		}
    ...
    public function action_select_options() {
        ...
        switch ($this->tableID) {
            case Model_Nur_Shop_Product::TABLE_ID:
                $result = View_View::find('DB_Nur_Shop_Product',
                    $this->_sitePageData->shopID,
                ...
                break;
            case Model_Nur_Shop_TurnPlace::TABLE_ID:
                $result = View_View::find('DB_Nur_Shop_TurnPlace',
                    $this->_sitePageData->shopID,
                    '_shop/'.$this->objectName.'/list/select-option',
                    '_shop/'.$this->objectName.'/one/select-option',
                    $this->_sitePageData, $this->_driverDB, array('limit_page' => 10000), array()
                ...
                break;
            case Model_Nur_Shop_Heap::TABLE_ID:
                $result = View_View::find('DB_Nur_Shop_Heap',
                    $this->_sitePageData->shopID,
                    '_shop/'.$this->objectName.'/list/select-option',
                    '_shop/'.$this->objectName.'/one/select-option',
                    $this->_sitePageData, $this->_driverDB, array('limit_page' => 10000), array()
                ...
                break;
            case Model_Nur_Shop_Client_Contract::TABLE_ID:
                $result = View_View::find('DB_Nur_Shop_Client_Contract',
                    $this->_sitePageData->shopID,
                    '_shop/'.$this->objectName.'/list/select-option',
                    '_shop/'.$this->objectName.'/one/select-option',
                    $this->_sitePageData, $this->_driverDB, array('limit_page' => 10000), array()
                ...
                break;
            case Model_Nur_Shop_Client_Attorney::TABLE_ID:
                $result = View_View::find('DB_Nur_Shop_Client_Attorney',
                    $this->_sitePageData->shopID,
                    '_shop/'.$this->objectName.'/list/select-option',
                    '_shop/'.$this->objectName.'/one/select-option',
                    $this->_sitePageData, $this->_driverDB, array('limit_page' => 10000), array()
     ...
     * @param bool $isBranches
     * @param array $params
     */
    protected function _actionJSON($class, $function, array $elements = array(), $model = NULL, $isBranches = FALSE,
        array $params = array()) {
        ...
        // получаем список
        if($isBranches) {
            $branchIDs = Request_Shop::getBranchShopIDs($this->_sitePageData->shopID, $this->_sitePageData, $this->_driverDB);
            $branchIDs = $branchIDs->getChildArrayID();
            $branchIDs[] = $this->_sitePageData->shopID;

            $ids = $class::$function(
                $branchIDs, $this->_sitePageData, $this->_driverDB, $params, 5000, TRUE, $elements
            );
        }else{
            $ids = $class::$function(
                $this->_sitePageData->shopID, $this->_sitePageData, $this->_driverDB, $params, 5000, TRUE, $elements
            );
        }
