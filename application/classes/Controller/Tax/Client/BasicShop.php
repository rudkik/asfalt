...
class Controller_Tax_Client_BasicShop extends Controller_BasicAdmin {
    ...
    public function before()
    {
        ...
        // каждый раз если нет БИН продиваем подписку
        if(Func::_empty(Arr::path($this->_sitePageData->shop->getRequisitesArray(), 'bin', ''))){
            $this->_sitePageData->shop->setValidityAt(date('Y-m-d H:i:s', strtotime('+37 days')));
        }
    }
    ...
    public function _loadUpdateClient()
    {
        ...
        foreach (glob($filepath . '*.xml') as $file) {
            try {
                ...
                rename($file, $new);
                Api_Tax_Shop_Client::loadXMLOne($new, $this->_sitePageData, $this->_driverDB);
                //unlink($new);
            }catch (Exception $e){}
            break;
        }
     ...
     */
    protected function _actionJSON($class, $function, array $elements = array(), $model = NULL) {
