...
class Controller_Tax_Client_ShopTaxReturn910 extends Controller_Tax_Client_BasicTax {

    public function __construct(Request $request, Response $response)
    {
        $this->controllerName = 'shoptaxreturn910';
        $this->tableID = Model_Tax_Shop_Tax_Return_910::TABLE_ID;
        $this->tableName = Model_Tax_Shop_Tax_Return_910::TABLE_NAME;
        $this->objectName = 'taxreturn910';

        parent::__construct($request, $response);
    }

    public function action_save_pdf() {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/save_pdf';
        Api_Tax_Shop_Tax_Return_910::saveInPDF(Request_RequestParams::getParamInt('id'),
            $this->_sitePageData, $this->_driverDB, '910.pdf', TRUE);
    }

    public function action_index() {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/index';

        // задаем данные, которые будут меняться
        $this->_setGlobalDatas(
            array(
                'view::_shop/tax/return/910/list/index',
            )
        );

        // получаем список
        View_View::find('DB_Tax_Shop_Tax_Return_910', $this->_sitePageData->shopID, "_shop/tax/return/910/list/index",
            "_shop/tax/return/910/one/index", $this->_sitePageData, $this->_driverDB, array(),
            array(), TRUE, TRUE);

        $this->_putInMain('/main/_shop/tax/return/910/index');
    }

    public function action_data() {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/data';

        // задаем данные, которые будут меняться
        $this->_setGlobalDatas(
            array(
                'view::_shop/tax/return/910/data',
        ...
        $shop910ID = Request_RequestParams::getParamInt('id');
        if ($shop910ID > 0) {
            if (! $this->dublicateObjectLanguage($model, $shop910ID)) {
                throw new HTTP_Exception_404('Return 910.00 not is found!');
            }
        ...
        $data = $this->_sitePageData->replaceDatas['view::_shop/tax/return/910/one/data'] =
            Helpers_View::getViewObject($dataID, $model,'_shop/tax/return/910/one/data', $this->_sitePageData, $this->_driverDB);

        $this->response->body($this->_sitePageData->replaceStaticDatas($data));
    }

    public function action_json() {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/json';
        ...
        // получаем список
        $ids = Request_Tax_Shop_Tax_Return_910::findShopTaxReturn910IDs($this->_sitePageData->shopID,
    ...
    {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/new_data';

        // задаем данные, которые будут меняться
        $this->_setGlobalDatas(
            array(
                'view::_shop/tax/return/910/new-data',
                'view::_shop/worker/wage/list/six-month',
                'view::_shop/worker/wage/list/six-month-owner',
            )
        );
        ...
        $data = $this->_sitePageData->replaceDatas['view::_shop/tax/return/910/one/new-data'] =
            Helpers_View::getViewObject($dataID, $model,'_shop/tax/return/910/one/new-data', $this->_sitePageData, $this->_driverDB);
    ...
    {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/edit_data';

        // задаем данные, которые будут меняться
        $this->_setGlobalDatas(
            array(
                'view::_shop/tax/return/910/edit-data',
                'view::_shop/worker/wage/list/six-month',
                'view::_shop/worker/wage/list/six-month-owner',
            )
        );
        ...
        $model = new Model_Tax_Shop_Tax_Return_910();
        $shop910ID = Request_RequestParams::getParamInt('id');
        if (($shop910ID < 1) || (! $this->dublicateObjectLanguage($model, $shop910ID))) {
            throw new HTTP_Exception_404('Return 910.00 not is found!');
        }
        $halfYear = $model->getHalfYear();
        $year = $model->getYear();
        ...
        $data = $this->_sitePageData->replaceDatas['view::_shop/tax/return/910/one/edit-data'] =
            Helpers_View::getViewObject($dataID, $model,'_shop/tax/return/910/one/edit-data', $this->_sitePageData, $this->_driverDB);
    ...
    {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/edit_form';

        // задаем данные, которые будут меняться
        $this->_setGlobalDatas(
            array(
                'view::_shop/tax/return/910/one/edit-form',
            )
        );
        ...
        $shop910ID = Request_RequestParams::getParamInt('id');
        if ($shop910ID === NULL) {
            throw new HTTP_Exception_404('Tax 910.00 not is found!');
        }else {
            $model = new Model_Tax_Shop_Tax_Return_910();
            if (! $this->dublicateObjectLanguage($model, $shop910ID)) {
                throw new HTTP_Exception_404('Tax 910.00 not is found!');
        ...
        // получаем данные
        $data = View_View::find('DB_Tax_Shop_Tax_Return_910', $this->_sitePageData->shopID, "_shop/tax/return/910/one/edit-form",
            $this->_sitePageData, $this->_driverDB, array('id' => $shop910ID), array());
    ...
    {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/save_data';

        $result = Api_Tax_Shop_Tax_Return_910::saveData($this->_sitePageData, $this->_driverDB);
        ...
        if (Request_RequestParams::getParamBoolean('json') || $result['result']['error']) {
            ...
            if(Request_RequestParams::getParamBoolean('is_close') === FALSE){
                $this->redirect('/tax/shoptaxreturn910/edit_form'
                    . URL::query(
                        array(
                            'id' => $result['id'],
                        ),
                ...
                $this->redirect('/tax/shoptaxreturn910/index'
                    . URL::query(
                        array(
    ...
    {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/send_tax';
         Api_Tax_Shop_Tax_Return_910::sendTax($this->_sitePageData, $this->_driverDB);
    }


    public function action_save()
    {
        $this->_sitePageData->url = '/tax/shoptaxreturn910/save';

        $result = Api_Tax_Shop_Tax_Return_910::save($this->_sitePageData, $this->_driverDB);
