...
class Controller_Sladushka_Main_Shopuser extends Controller_Sladushka_Main_BasicSladushka {
    ...
    public function action_auth() {
        $this->_sitePageData->url = '/sladushka/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/sladushka/shopuser/login';

        $email = Request_RequestParams::getParamStr('email');
        $password = Request_RequestParams::getParamStr('password');

        $auth = Auth::instance();
        $auth->setKeySession('sladushka');
        $auth->driverDB = $this->_driverDB;
        $auth->model = new Model_Shop_Operation();

        if (!$auth->login($email, $password)) {
            $this->redirect('/sladushka/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/sladushka");
    }
}
