...
class Controller_Sladushka_Manager_BasicShop extends Controller_BasicAdmin {
	...
	protected function _readUserInterface()
	{
		...
		if (!($auth->isLoginSessionShop())){
            ...
            if((empty($email)) || (empty($password))){
                $this->redirect('/manager?error=1');
			}else {
                if (!$auth->login($email, $password)) {
                    $this->redirect('/manager?error=2');
                }else{
	...
	public function isAccess(){
		$this->_sitePageData->actionURLName = 'manager';
	...
	{
		$this->_sitePageData->shopShablonPath = 'manager';
