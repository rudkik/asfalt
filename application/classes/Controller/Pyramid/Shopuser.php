...
class Controller_Pyramid_Shopuser extends Controller_Pyramid_BasicPyramid {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'pyramid';
        // данные о языке
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/pyramid/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/pyramid/shopuser/login';

        $email = Request_RequestParams::getParamStr('email');
        $password = Request_RequestParams::getParamStr('password');

        $auth = Auth::instance();
        $auth->setKeySession('pyramid');
        $auth->driverDB = $this->_driverDB;
        $auth->model = new Model_Pyramid_Shop_Client();

        if (!$auth->login($email, $password)) {
            $this->redirect('/pyramid/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            $this->redirect('/pyramid/shoppayment/index');
        }
    }
    ...
    public function action_registration() {
        ...
        $this->_sitePageData->url = '/pyramid/shopuser/registration';
        ...
        // генерируем основную часть
        $view = View::factory('pyramid/' . $this->_sitePageData->languageID . '/main/shopuser-registration');
        $view->data = array();
        $view->siteData = $this->_sitePageData;
        $result = Helpers_View::viewToStr($view);
        ...
        $view = View::factory('pyramid/' . $this->_sitePageData->languageID . '/index');
        $view->data = array();
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/pyramid");
    }

    public function action_resetpassword() {
        $this->_sitePageData->url = '/pyramid/shopuser/resetpassword';
