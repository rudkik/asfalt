...
class Controller_Pyramid_BasicShop extends Controller_BasicAdmin {
    ...
    public function _putInMain($file, $isMainNot = FALSE)
    {
        $this->_setGlobalDatas(
            array(
                'view::_shop/payment/list/menu-product',
            )
        );
        $params = Request_RequestParams::setParams(
            array('shop_client_id' => $this->_sitePageData->operationID)
        );
        View_View::find('DB_Pyramid_Shop_Payment',
            $this->_sitePageData->shopID,
            "_shop/payment/list/menu-product", "_shop/payment/one/menu-product",
    ...
    public function _loadUpdateClient()
    {
        ...
        foreach (glob($filepath . '*.xml') as $file) {
            try {
                ...
                rename($file, $new);
                Api_Pyramid_Shop_Client::loadXMLOne($new, $this->_sitePageData, $this->_driverDB);
                //unlink($new);
            }catch (Exception $e){}
            break;
        }
