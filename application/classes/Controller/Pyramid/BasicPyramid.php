...
class Controller_Pyramid_BasicPyramid extends Controller_Pyramid_BasicList
{
    public function __construct(Request $request, Response $response)
    {
        $this->actionURLName = 'pyramid';
    ...
    public function before()
    {
        ...
        try {
            $time = file_get_contents($path.$this->_sitePageData->shopID.'_pyramid.txt');
        }catch(Exception $e){
            $time = FALSE;
        }
        ...
        if((! $time) || ($newTime - strtotime($time) > 5 * 60)) {
            //Api_Pyramid_Shop_Bill::deleteRevise($this->_sitePageData, $this->_driverDB);
            file_put_contents($path.$this->_sitePageData->shopID.'_pyramid.txt', date('Y-m-d H:i:s', $newTime));
        }
    }
}