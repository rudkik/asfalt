...
class Controller_Finance_BasicList extends Controller_Finance_BasicShop
{
    /**
                                      ...
                                      $file = 'list', $field = 'name')
    {
        ...
        if(file_exists(VIEWPATH . 'finance/admin/' . $this->_sitePageData->dataLanguageID . '/' . $view . 'list/list.php')){
            $template = 'finance/admin';
            $viewPath = $view;
        }else{
            $template = 'finance/_all';
            $viewPath = '_db/';
        }
