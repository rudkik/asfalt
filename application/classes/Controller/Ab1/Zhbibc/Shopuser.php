...
class Controller_Ab1_ZhbiBc_Shopuser extends Controller_Ab1_ZhbiBc_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/zhbibc';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/zhbibc/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/zhbibc/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/zhbibc/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/zhbibc/admin/index');
            }else {
                $this->redirect('/zhbibc/shopcar/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/zhbibc");
    }
}
