...
class Controller_Ab1_Workflow_Shopuser extends Controller_Ab1_Workflow_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/workflow';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/workflow/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/workflow/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/workflow/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/workflow/admin/index');
            }else {
                $this->redirect('/workflow/shopcar/entry');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/workflow");
    }
}
