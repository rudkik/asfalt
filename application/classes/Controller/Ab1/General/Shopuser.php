...
class Controller_Ab1_General_Shopuser extends Controller_Ab1_General_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/general';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/general/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/general/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/general/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/general/admin/index');
            }else {
                $this->redirect('/general/shopcar/ttn');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/general");
    }
}
