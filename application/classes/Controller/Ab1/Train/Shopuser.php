...
class Controller_Ab1_Train_Shopuser extends Controller_Ab1_Train_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/train';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/train/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/train/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/train/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/train/admin/index');
            }else {
                $interfaceIDs = Arr::path($auth->model->getOptionsArray(), 'interface_ids', array());
                if($auth->model->getShopTableRubricID() == Model_Ab1_Shop_Operation::RUBRIC_TRAIN
                    || array_search(Model_Ab1_Shop_Operation::RUBRIC_TRAIN, $interfaceIDs) !== false) {
                    $this->redirect('/train/shopboxcartrain/index');
                }else{
                    $this->redirect('/train');
                }
            }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/train");
    }
}
