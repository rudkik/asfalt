...
class Controller_Ab1_Nbu_Shopuser extends Controller_Ab1_Nbu_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/nbu';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/nbu/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/nbu/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/nbu/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/nbu/admin/index');
            }else {
                $interfaceIDs = Arr::path($auth->model->getOptionsArray(), 'interface_ids', array());
                if($auth->model->getShopTableRubricID() == Model_Ab1_Shop_Operation::RUBRIC_NBU
                    || array_search(Model_Ab1_Shop_Operation::RUBRIC_NBU, $interfaceIDs) !== false) {
                    $this->redirect('/nbu/shopreport/index');
                }else{
                    $this->redirect('/nbu');
                }
            }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/nbu");
    }
}
