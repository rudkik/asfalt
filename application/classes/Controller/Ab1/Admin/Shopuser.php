...
class Controller_Ab1_Admin_Shopuser extends Controller_Ab1_Admin_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/admin';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/ab1-admin/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/ab1-admin/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/ab1-admin/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/ab1-admin/admin/index');
            }else {
                $this->redirect('/ab1-admin/shopcar/history');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/ab1-admin");
    }
}
