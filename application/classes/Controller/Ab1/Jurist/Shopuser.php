...
class Controller_Ab1_Jurist_Shopuser extends Controller_Ab1_Jurist_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/jurist/';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/jurist/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/jurist/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/jurist/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/jurist/admin/index');
            }else {
                $interfaceIDs = Arr::path($auth->model->getOptionsArray(), 'interface_ids', array());
                if($auth->model->getShopTableRubricID() == Model_Ab1_Shop_Operation::RUBRIC_JURIST
                    || array_search(Model_Ab1_Shop_Operation::RUBRIC_JURIST, $interfaceIDs) !== false) {
                    $this->redirect('/jurist/shopclientcontract/index');
                }else{
                    $this->redirect('/jurist/');
                }
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/jurist/");
    }
}
