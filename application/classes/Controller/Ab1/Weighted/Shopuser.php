...
class Controller_Ab1_Weighted_Shopuser extends Controller_Ab1_Weighted_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/weighted';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/weighted/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/weighted/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/weighted/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/weighted/admin/index');
            }else {
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/weighted");
    }
}
