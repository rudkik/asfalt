...
class Controller_Ab1_Peo_ShopTransportWaybill extends Controller_Ab1_Peo_BasicAb1 {
    ...
    public function action_index() {
        $this->_sitePageData->url = '/peo/shoptransportwaybill/index';
    ...
    {
        $this->_sitePageData->url = '/peo/shoptransportwaybill/edit';
    ...
    {
        $this->_sitePageData->url = '/peo/shoptransportwaybill/duplicate';
    ...
    {
        $this->_sitePageData->url = '/peo/shoptransportwaybill/save';
        ...
        if(Request_RequestParams::getParamBoolean('is_duplicate')){
            $this->redirect('/peo/shoptransportwaybill/duplicate?id='.$model->id);
        }else {
    ...
    public function action_get_finish_date(){
        $this->_sitePageData->url = '/peo/shoptransportwaybill/get_finish_date';
    ...
    public function action_refresh_cars(){
        $this->_sitePageData->url = '/peo/shoptransportwaybill/refresh_cars';
    ...
    public function action_calc_fuel(){
        $this->_sitePageData->url = '/peo/shoptransportwaybill/calc_fuel';
    ...
    public function action_car_works() {
        $this->_sitePageData->url = '/peo/shoptransportwaybill/car_works';
        ...
        $shopTransportIDs = array_merge(
            ...
            Request_RequestParams::getParamArray('trailers', array(), array())
        );

        $this->_sitePageData->newShopShablonPath($this->editAndNewBasicTemplate);
        $result = '';
        foreach ($shopTransportIDs as $shopTransportID) {
            $shopTransportID = intval($shopTransportID);
            ...
            $model->setDBDriver($this->_driverDB);
            if (! Helpers_DB::getDBObject($model, $shopTransportID, $this->_sitePageData)) {
    ...
    /**
     * Обновить задание водителю и список перевозок
     */
    public function action_refresh_cars_waybills()
    {
        $this->_sitePageData->url = '/peo/shoptransportwaybill/refresh_cars_waybills';

        $waybills = Request_Request::find(
            DB_Ab1_Shop_Transport_Waybill::NAME, 0,
            $this->_sitePageData, $this->_driverDB
        );
        ...
        foreach ($waybills->childs as $child){
            ...
            Api_Ab1_Shop_Transport_Waybill::refreshCars(
                ...
                $model->getFromAt(), $model->getToAt(), $this->_sitePageData, $this->_driverDB
            );
