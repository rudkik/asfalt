...
class Controller_Ab1_Peo_Shopuser extends Controller_Ab1_Peo_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/peo';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/peo/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/peo/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/peo/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/peo/admin/index');
            }else {
                $this->redirect('/peo/shopcar/history');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/peo");
    }
}
