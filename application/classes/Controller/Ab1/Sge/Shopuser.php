...
class Controller_Ab1_Sge_Shopuser extends Controller_Ab1_Sge_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/sge';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/sge/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/sge/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/sge/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/sge/admin/index');
            }else {
                $this->redirect('/sge/shopclientcontract/index?client_contract_status_id=1');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/sge");
    }
}
