...
class Controller_Ab1_Kpp_Shopuser extends Controller_Ab1_Kpp_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/kpp';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/kpp/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/kpp/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/kpp/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/kpp/admin/index');
            }else {
                $this->redirect('/kpp/shopreport/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/kpp");
    }
}
