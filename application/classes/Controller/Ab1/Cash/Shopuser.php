...
class Controller_Ab1_Cash_Shopuser extends Controller_Ab1_Cash_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/cash';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/cash/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/cash/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/cash/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/cash/admin/index');
            }else {
                $this->redirect('/cash/shopcar/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/cash");
    }
}
