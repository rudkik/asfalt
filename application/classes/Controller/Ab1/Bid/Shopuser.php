...
class Controller_Ab1_Bid_Shopuser extends Controller_Ab1_Bid_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/bid';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/bid/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/bid/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/bid/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/bid/admin/index');
            }else {
                $interfaceIDs = Arr::path($auth->model->getOptionsArray(), 'interface_ids', array());
                if($auth->model->getShopTableRubricID() == Model_Ab1_Shop_Operation::RUBRIC_BID
                    || array_search(Model_Ab1_Shop_Operation::RUBRIC_BID, $interfaceIDs) !== false) {
                    $this->redirect('/bid/shopplan/index');
                }else{
                    $this->redirect('/bid');
                }
            }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/bid");
    }
}
