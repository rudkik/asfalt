...
class Controller_Ab1_Ogm_Shopuser extends Controller_Ab1_Ogm_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/ogm';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/ogm/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/ogm/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/ogm/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/ogm/admin/index');
            }else {
                $this->redirect('/ogm/shopcar/asu');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/ogm");
    }
}
