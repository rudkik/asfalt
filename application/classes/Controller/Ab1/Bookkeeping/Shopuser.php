...
class Controller_Ab1_Bookkeeping_Shopuser extends Controller_Ab1_Bookkeeping_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/bookkeeping';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/bookkeeping/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/bookkeeping/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/bookkeeping/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/bookkeeping/admin/index');
            }else {
                $this->redirect('/bookkeeping/shoppricelist/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/bookkeeping");
    }
}
