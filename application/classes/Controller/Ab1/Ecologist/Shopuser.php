...
class Controller_Ab1_Ecologist_Shopuser extends Controller_Ab1_Ecologist_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/ecologist';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/ecologist/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/ecologist/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/ecologist/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/ecologist/admin/index');
            }else {
                $this->redirect('/ecologist/shopcar/asu');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/ecologist");
    }
}
