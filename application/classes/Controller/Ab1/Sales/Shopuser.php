...
class Controller_Ab1_Sales_Shopuser extends Controller_Ab1_Sales_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/sales';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/sales/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/sales/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/sales/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/sales/admin/index');
            }else {
                $this->redirect('/sales/shopreport/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/sales");
    }
}
