...
class Controller_Ab1_Sbyt_Shopuser extends Controller_Ab1_Sbyt_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/sbyt';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/sbyt/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/sbyt/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/sbyt/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/sbyt/admin/index');
            }else {
                $this->redirect('/sbyt/shopreport/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/sbyt");
    }
}
