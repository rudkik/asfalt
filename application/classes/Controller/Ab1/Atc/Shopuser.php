...
class Controller_Ab1_Atc_Shopuser extends Controller_Ab1_Atc_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/atc';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/atc/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/atc/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/atc/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/atc/admin/index');
            }else {
                $this->redirect('/atc/shoptransportwaybill/index');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/atc");
    }
}
