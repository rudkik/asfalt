...
class Controller_Ab1_Asu_Shopuser extends Controller_Ab1_Asu_BasicAb1 {
    public function before() {
        $this->_sitePageData->shopShablonPath = 'ab1/asu';
        $this->_readLanguageInterface();
    }

    public function action_auth() {
        $this->_sitePageData->url = '/asu/shopuser/auth';
    ...
    public function action_login() {
        $this->_sitePageData->url = '/asu/shopuser/login';
        ...
        if (!$auth->login($email, $password)) {
            $this->redirect('/asu/shopuser/auth?is_main='.Request_RequestParams::getParamBoolean('is_main').'&s=1');
        } else {
            if($auth->model->getShopTableRubricID() == 0){
                $this->redirect('/asu/admin/index');
            }else {
                $this->redirect('/asu/shopcar/shipment');
            }
        }
    }
    ...
    public function action_unlogin() {
        ...
        $this->redirect($this->_sitePageData->urlBasic . "/asu");
    }
}
