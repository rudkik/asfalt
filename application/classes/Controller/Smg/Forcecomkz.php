...
class Controller_Smg_ForcecomKZ extends Controller_BasicControler {
    ...
    public function action_load_rubrics() {
        $this->_sitePageData->url = '/smg/forcecomkz/load_rubrics';
        ...
        $data = Helpers_URL::getPageHTMLRandomProxy(
            Drivers_ParserSite_ForcecomKZ::URL_RUBRICS, Helpers_Path::getFilesProxies()
        );
        //$data = file_get_contents('C:\WAMP\www\ct\al_style_catalog.xml');

        preg_match_all('/class="section1" title=".+" href="(.+)">/U', $data, $result);
        if (count($result) < 2) {
        ...
        foreach($result[1] as $url) {
            $url = $this->_sitePageData->urlBasic . '/smg/forcecomkz/load_rubric' . URL::query(
                [
                    'rubric' => 'https://www.forcecom.kz' . $url,
    ...
    public function action_load_rubric() {
        $this->_sitePageData->url = '/smg/forcecomkz/load_rubric';
        ...
        $urls = Drivers_ParserSite_ForcecomKZ::getProductURLsByRubric(
    ...
    public function action_load_rubrics_products() {
        $this->_sitePageData->url = '/smg/forcecomkz/load_rubrics_products';
        set_time_limit(3600000);
        ini_set("max_execution_time", "3600000");
        ...
        foreach ($shopSupplierParserIDs->childs as $child) {
            $url = $this->_sitePageData->urlBasic . '/smg/forcecomkz/load_rubric_products' . URL::query(
                    [
    ...
    public function action_load_rubric_products() {
        $this->_sitePageData->url = '/smg/forcecomkz/load_rubric_products';
        set_time_limit(3600000);
        ini_set("max_execution_time", "3600000");
        ...
        foreach ($shopProductIDs->childs as $child) {
            ...
            if($model->getIsPublic()){
                ...
                Drivers_ParserSite_ForcecomKZ::loadProduct($url, $model, $this->_sitePageData,  $this->_driverDB, $fileProxies, false);
