...
class Kohana_Image_GD extends Image {
     ...
     * @return  void
     */
    protected function _do_resize_original($width, $height)
    {
     ...
     * @return  void
     */
    protected function _do_resize($width, $height, $shiftX = 0, $shiftY = 0)
    {
        // Presize width and height
        $pre_width = $this->width;
        $pre_height = $this->height;
        ...
        if ($width > ($this->width / 2) AND $height > ($this->height / 2))
        {
            ...
            if(($width > $pre_width) && ($height > $pre_height)){
                $reduction_width = round($pre_width * 1.1);
                $reduction_height = round($pre_height * 1.1);
            }else {
                $reduction_width = round($width * 1.1);
                $reduction_height = round($height * 1.1);
            }
        ...
        // Create the temporary image to copy to
        $image = $this->_create($width + $shiftX * 2, $height + $shiftY * 2);

        if(($shiftX > 0.9) || ($shiftY > 0.9)) {
            ...
            imagefilledrectangle($image, 0, 0, $width + $shiftX * 2, $height + $shiftY * 2, $color);
        }

        // нельзя увеличивать размер фото
        if((($shiftX > 0) || ($shiftY > 0)) && ($width > $this->width) && ($height > $this->height)){
            $shiftX = round(($width - $this->width + $shiftX * 2) / 2);
            $shiftY = round(($height - $this->height + $shiftY * 2) / 2);
        ...
        // Execute the resize
        if (imagecopyresampled($image, $this->_image, $shiftX, $shiftY, 0, 0, $width, $height, $pre_width, $pre_height))
        {
            // Swap the new image for the old one
            imagedestroy($this->_image);
            $this->_image = $image;
    ...
    protected function _do_save($file, $quality)
    {
        ...
        $this->extension = $extension = pathinfo($file, PATHINFO_EXTENSION);
