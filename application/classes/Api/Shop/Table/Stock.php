...
class Api_Shop_Table_Stock  {
     ...
     */
    public static function saveListCollations($shopTableStockTypeID, SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver)
    {
        $model = new Model_Shop_Table_Stock();
        $model->setDBDriver($driver);

        $shopTableStocks = Request_RequestParams::getParamArray('data', array());
        foreach ($shopTableStocks as &$shopTableStock) {
            $model->clear();

            $id = intval(Arr::path($shopTableStock, 'shop_table_stock_id', 0));
            if ($id > 0) {
                if (!Helpers_DB::dublicateObjectLanguage($model, $id, $sitePageData, $sitePageData->shopID)) {
                    continue;
                }
            } else {
                $model->setShopTableStockTypeID($shopTableStockTypeID);
            }

            if($id < 1) {
                if (key_exists('is_public', $shopTableStock)) {
                    $model->setIsPublic($shopTableStock['is_public']);
                }
                if (key_exists('collations', $shopTableStock)) {
                    $model->addCollationsArray($shopTableStock['collations']);
                }
                if (key_exists('old_id', $shopTableStock)) {
                    $model->setOldID($shopTableStock['old_id']);
                }
                if (key_exists('name', $shopTableStock)) {
                    if (!empty($shopTableStock['name'])) {
                        $model->setName($shopTableStock['name']);
                    }
                }
                if (key_exists('text', $shopTableStock)) {
                    $model->setText($shopTableStock['text']);
                }
            }

            $options = array();
            foreach($shopTableStock as $k => $v){
            ...
            $shopTableStock['shop_table_stock_id'] = Helpers_DB::saveDBObject($model, $sitePageData, $sitePageData->shopID);
            $shopTableStock['shop_table_stock_name'] = $model->getName();
        }

        return $shopTableStocks;
    }
    ...
    public static function saveList(SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver)
    {
        ...
        $shopTableStocks = Request_RequestParams::getParamArray('shop_table_stocks', array());
        if ($shopTableStocks === NULL) {
            return FALSE;
        }

        foreach ($shopTableStocks as $id => $shopTableStock) {
            $id = intval($id);
            if ($id > 0) {
                if (!Helpers_DB::dublicateObjectLanguage($model, $id, $sitePageData, $sitePageData->shopID)) {
                    continue;
                ...
                if (key_exists('remarketing', $shopTableStock)) {
                    $model->setRemarketing($shopTableStock['remarketing']);
                }

                if (key_exists('shop_table_stock_id', $shopTableStock)) {
                    $model->setShopTableStockID($shopTableStock['shop_table_stock_id']);
                }

                if (key_exists('options', $shopTableStock)) {
                    $options = $shopTableStock['options'];
                    if (is_array($options)) {
                        $model->addOptionsArray($options);
                    }
                }
                if (key_exists('collations', $shopTableStock)) {
                    $options = $shopTableStock['collations'];
                    if (!is_array($options)) {
                        $options = explode("\r\n", $options);
                    }
    ...
    public static function save(SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver)
    {
        ...
        if($id > 0) {
            if (!Helpers_DB::dublicateObjectLanguage($model, $id, $sitePageData)) {
                throw new HTTP_Exception_500('Stock not found.');
            }
    ...
    public static function addImages(SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver)
    {
        ...
        if(key_exists('files', $_FILES)){
            ...
            foreach($files as $file){
                ...
                if($isBarcode === TRUE){
                    $ids = Request_Request::find('DB_Shop_Table_Stock', $sitePageData->shopID, $sitePageData, $driver,
