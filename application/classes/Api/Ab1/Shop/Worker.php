...
class Api_Ab1_Shop_Worker  {
    ...
    public static function save(SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver)
    {
        ...
        $shopBranchID = intval(Request_RequestParams::getParamInt('shop_branch_id_'));
        if($shopBranchID < 1){
            $shopBranchID = $sitePageData->shopID;
        }
        ...
        if ($model->validationFields($result)) {
            if($model->id < 1) {
                Helpers_DB::saveDBObject($model, $sitePageData, $shopBranchID);
            }

            // изменяем филиал
            if($shopBranchID != $model->shopID){
                $driver->sendSQL('UPDATE ab_shop_workers SET shop_id = '.$shopBranchID.' WHERE id='.$model->id);
            }
