...
class Api_Tax_Shop_Invoice_Proforma_Item  {

    /**
     * @param Model_Tax_Shop_Invoice_Proforma $modelInvoice
     * @param array $shopInvoiceProformaItems
     * @param SitePageData $sitePageData
     * @param Model_Driver_DBBasicDriver $driver
     * @return float|int
     */
    public static function save(Model_Tax_Shop_Invoice_Proforma $modelInvoice, array $shopInvoiceProformaItems,
                                SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver)
    {
        ...
        $model = new Model_Tax_Shop_Invoice_Proforma_Item();
        $model->setDBDriver($driver);

        $shopInvoiceProformaItemIDs = Request_Tax_Shop_Invoice_Proforma_Item::find($sitePageData->shopID, $sitePageData, $driver,
            array('shop_invoice_id' => $shopInvoiceProformaID, Request_RequestParams::IS_NOT_READ_REQUEST_NAME => TRUE), 0, TRUE);
        ...
        foreach($shopInvoiceProformaItems as $shopInvoiceProformaItem){
            $name = Arr::path($shopInvoiceProformaItem, 'shop_product_name', '');
            if(empty($name)){
                continue;
            }
            $quantity = str_replace(',', '.', str_replace(' ', '', floatval(Arr::path($shopInvoiceProformaItem, 'quantity', 0))));
            if($quantity <= 0){
                continue;
            }

            $isService = Request_RequestParams::isBoolean(Arr::path($shopInvoiceProformaItem, 'is_service', FALSE));
            $price = str_replace(',', '.', str_replace(' ', '', floatval(Arr::path($shopInvoiceProformaItem, 'price', 0))));

            // получаем единицу измерения
            $unit = trim(Arr::path($shopInvoiceProformaItem, 'unit_name', ''));
            if (!empty($unit)) {
            ...
            if($shopInvoiceProformaItemID !== NULL){
                $model->__setArray(array('values' => $shopInvoiceProformaItemID->values));
            }
        ...
        // удаляем лишние
        $driver->deleteObjectIDs($shopInvoiceProformaItemIDs->getChildArrayID(), $sitePageData->userID,
            Model_Tax_Shop_Invoice_Proforma_Item::TABLE_NAME, array(), $sitePageData->shopID);
