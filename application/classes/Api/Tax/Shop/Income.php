...
class Api_Tax_Shop_Income  {
    ...
    {
        $model = new Model_Tax_Shop_Income();
        $model->setDBDriver($driver);

        $id = Request_RequestParams::getParamInt('id');
        if($id > 0) {
            if (!Helpers_DB::dublicateObjectLanguage($model, $id, $sitePageData)) {
                throw new HTTP_Exception_500('Income not found.');
            }
            ...
            if($type > 0) {
                ...
                if ($hashtags !== NULL) {
                    $model->setShopTableHashtagIDsArray(Api_Shop_Table_ObjectToObject::saveToHashtags(
                        Model_Tax_Shop_Income::TABLE_ID, $model->id, $model->getShopTableCatalogID(),
