...
class View_Hotel_Shop_Room extends View_View {
    /**
     ...
     * @throws Exception
     */
    public static function findFreeShopRooms($dbObject, $shopID, $viewObjects, $viewObject, SitePageData $sitePageData,
                                             Model_Driver_DBBasicDriver $driver, array $params = array(), $elements = NULL,
                                             $isLoadView = TRUE, $isLoadOneView = FALSE){
        if (Model_Memcache_ShopMemcacheDriver::IS_SAVE_MEMCACHE) {
            $tables = self::getTables(Model_Hotel_Shop_Room::TABLE_NAME, $elements);
            $key = Helpers_DB::getURLParamDatas(array(), $params);
            $result = Helpers_DB::getMemcacheFunctionView($shopID, 'View_Hotel_Shop_Room::findFreeShopRooms', $tables, $viewObjects, $sitePageData, $driver, $key);
            if ($result !== NULL) {
        ...
        $ids = Api_Hotel_Shop_Room::getFreeRooms($dateFrom, $dateTo, $sitePageData, $driver, FALSE, $params);

        // нужно ли считать стоимость
        if (Request_RequestParams::getParamBoolean('is_calc_amount', $params)){
            $amount = 0;
            foreach ($ids->childs as $child){
                $tmp = Api_Hotel_Shop_Room::getAmountRoomOfValues($child->values['id'], array(), 0, 0,
        ...
        if (Model_Memcache_ShopMemcacheDriver::IS_SAVE_MEMCACHE) {
            Helpers_DB::setMemcacheFunctionView($result, $shopID, 'View_Hotel_Shop_Room::findFreeShopRooms', $tables, $viewObjects, $sitePageData, $driver, $key);
        }
