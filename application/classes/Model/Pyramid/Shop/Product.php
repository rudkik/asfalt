...
class Model_Pyramid_Shop_Product extends Model_Shop_Basic_Options{

	const TABLE_NAME = 'np_shop_products';
	const TABLE_ID = 240;
    ...
    public function dbGetElements($shopID = 0, $elements = NULL){
        if(($elements === NULL) || (! is_array($elements))){
            ...
            foreach($elements as $element){
                switch($element){
                    case 'unit_id':
                        $this->_dbGetElement($this->getUnitID(), 'unit_id', new Model_Pyramid_Unit());
                        break;
                }
            }
        }
