...
class Model_Ab1_Shop_Material_Rubric_Make extends Model_Shop_Table_Basic_Table{
    ...
    public function dbGetElements($shopID = 0, $elements = NULL){
        if(is_array($elements)){
            foreach($elements as $element){
                switch($element){
                    case 'root_id':
                        $this->_dbGetElement($this->getRootID(), 'root_id', new Model_Ab1_Shop_Material_Rubric_Make(), $shopID);
                        break;
                }
            }
        }
