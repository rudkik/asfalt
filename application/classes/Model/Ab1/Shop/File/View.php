...
class Model_Ab1_Shop_File_View extends Model_Shop_Basic_Object{

	const TABLE_NAME = 'ed_shop_file_views';
	const TABLE_ID = 127;
    ...
    public function dbGetElements($shopID = 0, $elements = NULL){
        if(($elements === NULL) || (! is_array($elements))){
            ...
            foreach($elements as $element){
                switch($element){
                    case 'shop_file_id':
                        $this->_dbGetElement($this->getShopFileID(), 'shop_file_id', new Model_Ab1_Shop_File_View());
                        break;
