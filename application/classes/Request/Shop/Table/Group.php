...
class Request_Shop_Table_Group extends Request_Request{
    /**
	...
	public static function findShopRootIDs($shopID, SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver,
        ...
        $sql->setTableName(Model_Shop_Table_Group::TABLE_NAME);
        $sql->limit = $limit;

        $sql->getRootSelect()->addField('', "shop_root_object_id", 'id');

        $sql->basicTableName = Model_Shop_Table_Group::TABLE_NAME;
        if(! static::_getRequestParamsSQL($shopID, $sql, $sitePageData, $driver, $isNotReadRequest, $params, $sortBy,
            $groupBy, Model_Shop_Table_Group::TABLE_NAME)){
            return new MyArray();
        }
        ...
        return Request_RequestUtils::ArrayInMyArrayList(
            $sitePageData, $resultSQL, $limit, $limitPage, $page, $isLoadAllFields, false, $isShift
        );
	}
