...
class Request_Shop_Table_ObjectToObject extends Request_Request{
    /**
    ...
    /**
     * Поиск родителей связей объектов
     * @param $shopID
	...
	public static function findShopRootIDs($shopID, SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver,
        ...
        $sql->setTableName(Model_Shop_Table_ObjectToObject::TABLE_NAME);
        $sql->limit = $limit;

        $sql->getRootSelect()->addField('', "shop_root_object_id", 'id');

        $sql->basicTableName = Model_Shop_Table_ObjectToObject::TABLE_NAME;
        if(! static::_getRequestParamsSQL($shopID, $sql, $sitePageData, $driver, $isNotReadRequest, $params, $sortBy,
            $groupBy, Model_Shop_Table_ObjectToObject::TABLE_NAME)){
            return new MyArray();
        }
        ...
        $resultSQL = $driver->getSelect($sql, $isLoadAllFields, -1, $shopID);
	 ...
	 * @return MyArray
	 */
	public static function findBranchShopRootIDs(array $shopIDs, SitePageData $sitePageData, Model_Driver_DBBasicDriver $driver,
										   array $params = array(), $limit = 0){
		...
		$sql->getRootSelect()->addField('', "shop_root_object_id", 'id');
		$sql->getRootWhere()->addField('', "shop_id", '', $shopIDs);

        $sql->basicTableName = Model_Shop_Table_ObjectToObject::TABLE_NAME;
		if(! static::_getRequestParamsSQL(0, $sql, $sitePageData, $driver, $isNotReadRequest, $params, $sortBy,
            $groupBy, Model_Shop_Table_ObjectToObject::TABLE_NAME)){
			return new MyArray();
		}
