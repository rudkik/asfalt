...
class Request_Tax_Shop_Invoice_Proforma extends Request_Request {
    /**
    ...
    protected static function _addWhere($dbObject, $fieldWhere, $fieldValue, array $params, Model_Driver_DBBasicSQL $sql,
        ...
        if($fieldWhere == 'search') {
            ...
            if (!empty($search)) {
                ...
                $tableJOIN = static::_addElementTableSQL(Model_Tax_Shop_Invoice_Proforma::TABLE_NAME, 'shop_contractor_id', Model_Tax_Shop_Contractor::TABLE_NAME, array('id'), $sql);
                $tmp->addField('name', $tableJOIN, $search, '', Model_Driver_DBBasicWhere::COMPARE_TYPE_LIKE);
                $tmp->addField('bin', $tableJOIN, $search, '', Model_Driver_DBBasicWhere::COMPARE_TYPE_EQUALLY);

                $tableJOIN = static::_addElementTableSQL(Model_Tax_Shop_Invoice_Proforma::TABLE_NAME, 'shop_contract_id', Model_Tax_Shop_Contract::TABLE_NAME, array('id'), $sql);
                $tmp->addField('number', $tableJOIN, $search, '', Model_Driver_DBBasicWhere::COMPARE_TYPE_EQUALLY);
            }
            return true;
        }
