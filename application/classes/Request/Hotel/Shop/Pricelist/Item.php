...
class Request_Hotel_Shop_Pricelist_Item extends Request_Request {
    /**
    ...
    protected static function _addWhere($dbObject, $fieldWhere, $fieldValue, array $params, Model_Driver_DBBasicSQL $sql,
        ...
        if($fieldWhere == 'date') {
            ...
            if(($date !== NULL)){
                $sql->getRootWhere()->addField('date_from', '', $date, '', Model_Driver_DBBasicWhere::COMPARE_TYPE_LESS_EQUALLY);
                $sql->getRootWhere()->addField('date_to', '', $date, '', Model_Driver_DBBasicWhere::COMPARE_TYPE_MORE_EQUALLY);
            }
