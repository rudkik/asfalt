...
class Request_Ab1_Shop_Defect_Car extends Request_Request {
    /**
    ...
    protected static function _addFromSQL($dbObject, $elementName, array $elementFields, Model_Driver_DBBasicSQL $sql, array $params){
        if($elementName == 'root_rubric_id') {
            $table = static::_addElementTableSQL(
                Model_Ab1_Shop_Defect_Car::TABLE_NAME, 'shop_product_id',
    ...
    protected static function _addGroupBy($dbObject, $groupByName, Model_Driver_DBBasicSQL $sql, $sortBy = NULL)
    {
        if($groupByName == 'exit_at_day_6_hour') {
            ...
            $sql->getRootSelect()->addFunctionField(
                '',
                'CASE WHEN date('.Model_Ab1_Shop_Defect_Car::TABLE_NAME.'.exit_at) + interval \'6 hour\' > '.Model_Ab1_Shop_Defect_Car::TABLE_NAME.'.exit_at THEN '.Model_Ab1_Shop_Defect_Car::TABLE_NAME.'.exit_at - interval \'1 day\''
                . ' ELSE ' . Model_Ab1_Shop_Defect_Car::TABLE_NAME.'.exit_at END',
                'date', 'exit_at_day'
            );
    ...
    protected static function _addWhere($dbObject, $fieldWhere, $fieldValue, array $params, Model_Driver_DBBasicSQL $sql,
        ...
        if($fieldWhere == 'exit_at_day_6_hour') {
            ...
            if(!empty($days)){
                ...
                foreach ($days as $day){
                    $tmp->addField(
                        'date(CASE WHEN date(' . Model_Ab1_Shop_Defect_Car::TABLE_NAME . '.exit_at) + interval \'6 hour\' > '
                        . Model_Ab1_Shop_Defect_Car::TABLE_NAME . '.exit_at THEN ' . Model_Ab1_Shop_Defect_Car::TABLE_NAME . '.exit_at - interval \'1 day\''
                        . ' ELSE ' . Model_Ab1_Shop_Defect_Car::TABLE_NAME . '.exit_at END)',
                        '', $day
                    )->isFuncField1 = true;
                }
        ...
        if($fieldWhere == 'is_night_22_to_6_hour') {
            ...
            if($isNight === true){
                ...
                $tmp->addField(
                    Model_Ab1_Shop_Defect_Car::TABLE_NAME . '.exit_at::time',
