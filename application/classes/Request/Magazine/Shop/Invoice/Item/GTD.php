...
class Request_Magazine_Shop_Invoice_Item_GTD extends Request_Request {
    /**
    ...
    protected static function _addFromSQL($dbObject, $elementName, array $elementFields, Model_Driver_DBBasicSQL $sql, array $params){
        ...
        if($elementName == 'shop_production_unit_id') {
            $table = static::_addElementTableSQL(
                Model_Magazine_Shop_Receive_Item::TABLE_NAME, 'shop_production_id',
    ...
    protected static function _addWhere($dbObject, $fieldWhere, $fieldValue, array $params, Model_Driver_DBBasicSQL $sql,
        ...
        if($fieldWhere == 'esf_type_id') {
            $esfTypeID = $fieldValue;
            if(($esfTypeID !== NULL)){
                if ((is_array($esfTypeID) && count($esfTypeID) > 0) || (intval($esfTypeID) > -1)) {
                    $tableJoin = $sql->getRootFrom()->addTable(
                        Model_Magazine_Shop_Invoice_Item_GTD::TABLE_NAME, 'shop_invoice_id',
