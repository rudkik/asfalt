...
abstract class Kohana_Image {
	 ...
	 */
	public function resize($width = NULL, $height = NULL, $master = NULL, $isWhiteBackground = FALSE)
	{
        $start_width = $width;
        if($start_width == NULL){
            $start_width = $this->width * $height / $this->height;
        }
        $start_height = $height;
        if($start_height == NULL){
            $start_height = $this->height * $width / $this->width;
        }
        ...
        if(($isWhiteBackground) && ($start_height > 0) && ($start_width > 0)) {
            ...
            $shiftX = round(($start_width - $width) / 2);
            $shiftY = round(($start_height - $height) / 2);

            // проверяем, чтобы размер ме превышал допустимые
            if ($shiftX * 2 + $width != $start_width) {
                $width = $width + ($start_width - ($shiftX * 2 + $width));
            }
            if ($shiftY * 2 + $height != $start_height) {
                $height = $height + ($start_height - ($shiftY * 2 + $height));
            }

            if (($isWhiteBackground)) {
                $this->_do_resize($width, $height, $shiftX, $shiftY);
            } else {
                $this->_do_resize($width, $height);
            }
        }else{
            $this->_do_resize_original($width, $height);
        }
