...
function __init(){
    ...
    $('.table td li.tr-remove a[delete="tr"]').click(function () {
    ...
    $('select[type="attribute_catalog_ids"]').change(function () {
        ...
        input.attr('name', 'attr_catalog_ids[' + $(this).val() + ']');
        input.attr('list', 'attribute_names_' + $(this).val());
    });
    ...
    $('[data-action="copy-price"]').click(function () {
        id = $(this).data('id');
        val = $('#'+id).val();

        $('#price_a').val(val);
        $('#price_b').val(val);
        $('#price_b_plus').val(val);
        $('#price_c').val(val);
        $('#price_market').val(val);
        $('#price_horeca').val(val);
...
function editBill(element, price, id) {
    ...
    $('#goods-'+id).text((price * count) + ' тг');

    amount = 0;
    $('#list-goods input[data-action="count-edit"]').each(function (i) {
...
$(document).ready(function () {
    ...
    $('#find-goods-id button[type="submit"]').click(function () {
        actionFindGoods('find-goods-id', 'result-goods', 0, '/sadmin/shopgood/findpromo');
    ...
    $('#find-news-similar-id button[type="submit"]').click(function () {
        actionFindGoods('find-news-similar-id', 'result-news', 0, '/sadmin/shopnew/findsimilar');
    ...
    $('#find-goods-similar-id button[type="submit"]').click(function () {
        actionFindGoods('find-goods-similar-id', 'result-goods-similar', 0, '/sadmin/shopgood/findsimilar');
    ...
    $('#find-goods-gift-id button[type="submit"]').click(function () {
        actionFindGoods('find-goods-gift-id', 'result-goods-gift', 0, '/sadmin/shopgood/findpromogift');
    ...
    $('#find-goods-group-id button[type="submit"]').click(function () {
        actionFindGoods('find-goods-id', 'result-goods', 0, '/sadmin/shopgood/findgoodgroup');
...
function actionAddTR(bodyPanelID, trPanelID, removePanelID, postfix, inputName) {
    $('#panel-news-postfix').text(postfix);
    $('#panel-news-postfix').data('input-name', inputName);
...
function delShopBillItem(shopID, billItemID) {
    var s = $('#main_panel');
    jQuery.ajax({
        url: '/sadmin/shopbillitem/del',
        data: ({
            shop_id: (shopID),
            id: (billItemID),
            is_main: (1),
            url: ('/sadmin/shopbill/edit'),
        }),
        type: "POST",
        success: function (data) {
            s.html(data);
...
function initialize() {
 ...
 /*   buzz.defaults.formats = ['ogg', 'mp3'];
    ...
    if (buzz.isSupported()) {
        var noticeSound = new buzz.sound("/css/_component/loadimage_v2/sounds/notice");
    }
