...
function __initTr() {
    ...
    if (field.length > 0) {
        ...
        var products = new Bloodhound({
            ...
            prefetch: '/pyramid/shoproom/json?limit=15',
            remote: {
                url: '/pyramid/shoproom/json?name=%QUERY&sort_by[name]=asc&limit=25&_fields[]=name',
                wildcard: '%QUERY'
            }
        });
        field.typeahead({
...
function __initTable() {
    ...
    fields = $('table[data-action="nur-table"]:not([data-nur-table="1"])');
    if (fields.length > 0) {
        fields.attr('data-nur-table', 1);
    ...
    if (field.length > 0) {
        ...
        var bills = new Bloodhound({
            datumTokenizer: Bloodhound.tokenizers.obj.whitespace('name'),
            queryTokenizer: Bloodhound.tokenizers.whitespace,
            prefetch: '/pyramid/shopbill/json?limit=15',
            remote: {
                url: '/pyramid/shopbill/json?id=%QUERY&sort_by[name]=asc&limit=25&_fields[]=id&_fields[]=shop_client_name&_fields[]=shop_client_id',
                wildcard: '%QUERY'
            }
        });
        field.typeahead({
    ...
    if (field.length > 0) {
        ...
        var products = new Bloodhound({
            ...
            prefetch: '/pyramid/shoproom/json?limit=15',
            remote: {
                url: '/pyramid/shopclient/json?name=%QUERY&sort_by[name]=asc&limit=25&_fields[]=name&_fields[]=id',
                wildcard: '%QUERY'
            }
        });
        field.typeahead({
...
function getDateTime(value, row) {
